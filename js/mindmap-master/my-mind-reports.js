...
MM.Item.prototype.fromJSON = function(data) {
	...
	if (data.items_id) { this._items_id = data.items_id; }
	if (data.popup) { this._popup = data.popup; }
	if (data.status) {
...
MM.Item.prototype.update = function(doNotRecurse, is_zoom) {
	var map = this.getMap();
	...
	{ 
		this._parent.update(doNotRecurse, is_zoom);				
	}

	//console.log(is_zoom)
	//set updated flag
	if(is_zoom==null && MM.App.is_editable)
	{	
		$('#is_mind_map_updated').val(1)
		
		if(!$('#mm_btn_save').hasClass('btn-primary'))
		{
			$('#mm_btn_save').addClass('btn-primary')
		}
...
MM.Item.prototype.updateSubtree = function(isSubChild,is_zoom) {
	this._children.forEach(function(child) {
		child.updateSubtree(true,is_zoom);
	});
	return this.update(isSubChild,is_zoom);
}
...
MM.Item.prototype.setText = function(text) {
	...
	//prepare new entity item
	if(this._id!='root' && this._parent && this._items_id==null)
	{						
		var obj = this;
		
		$.ajax({
		  method: "POST",
		  url: $('#data-item-prepare-new').val(),
		  data: { mm_id: this._id, text: text, parent_id: this._parent._id}
		})
		  .done(function(data) {
		  	...
		  	if(data.length)
		  	{
		  		...
		  		obj._items_id = data.item_id;
		  		obj._color = data.color;		  		
		  		obj._popup = data.popup;
		  		obj._icon = data.icon;
  ...
  if (icon)
	{
        ...
        this._dom.icon.setAttribute("style", "color:"+this._color);
        
        if(this._items_id)
        {	
        	this._dom.icon.setAttribute('target','_blank');
        	this._dom.icon.setAttribute('href',$('#data-item-url').val()+$('#data-item-path').val()+'-'+this._items_id);
        	
        	if(this._popup.length>0)
        	{	
        		this._dom.icon.setAttribute('data-toggle','popover');
        		this._dom.icon.setAttribute('data-content',this._popup);
        	}
        }
...
MM.Item.prototype._updateValue = function() {
	...
	*/
	...
	//console.log(this._computed.value);
...
MM.Map.prototype.update = function() {	
	this._root.updateSubtree(false, true);
	return this;
}
...
MM.Command.InsertSibling = Object.create(MM.Command, {
	label: {value: i18n["TEXT_INSERT_SIBLING"]},
...
MM.Command.InsertChild = Object.create(MM.Command, {
	label: {value: i18n["TEXT_INSERT_CHILD"]},
	keys: {value: [
...
MM.Command.Center = Object.create(MM.Command, {
	label: {value: i18n["TEXT_CENTER_MAP"]},
...
MM.Command.New.execute = function() {
	if (!confirm(i18n["TEXT_RESET_MAP_CONFIRM"])) { return; }
	var map = new MM.Map();
	MM.App.setMap(map);
...
MM.Command.Underline = Object.create(MM.Command.Style, {
	...
	label: {value: i18n["TEXT_UNDERLINE"]},
...
MM.Command.Value = Object.create(MM.Command, {
	label: {value: i18n["TEXT_SET_VALUE"]},
...
MM.Layout.Graph.Down = MM.Layout.Graph.create("bottom", "graph-bottom", i18n["TEXT_ON_BOTTOM"]);
MM.Layout.Graph.Up = MM.Layout.Graph.create("top", "graph-top", i18n["TEXT_ON_TOP"]);
MM.Layout.Graph.Left = MM.Layout.Graph.create("left", "graph-left", i18n["TEXT_ON_LEFT"]);
MM.Layout.Graph.Right = MM.Layout.Graph.create("right", "graph-right", i18n["TEXT_ON_RIGHT"]);
...
MM.Layout.Tree.Left = MM.Layout.Tree.create("left", "tree-left", i18n["TEXT_ON_LEFT"]);
MM.Layout.Tree.Right = MM.Layout.Tree.create("right", "tree-right", i18n["TEXT_ON_RIGHT"]);
MM.Layout.Map = Object.create(MM.Layout.Graph, {
	id: {value:"map"},
	label: {value:i18n["TEXT_MAP"]},
...
MM.Shape.Box = Object.create(MM.Shape, {
	id: {value: "box"},
	label: {value: i18n["TEXT_BOX"]}
});
MM.Shape.Ellipse = Object.create(MM.Shape, {
	id: {value: "ellipse"},
	label: {value: i18n["TEXT_ELLIPSE"]}
});
...
MM.UI.Layout = function() {
	...
	var label = this._buildGroup(i18n['TEXT_GRAPH']);
	...
	var label = this._buildGroup(i18n["TEXT_TREE"]);
