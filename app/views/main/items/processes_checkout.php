    ...
    <div class="form-body ajax-modal-width-790">
        ...
        if (count($payment_modules)) {
            ...
            foreach ($payment_modules as $modules_id => $modules_title) {
                if (!in_array($modules_id, explode(',', $app_process_info['payment_modules']))) {
                    continue;
                }
				...
				<div style="margin-bottom: 5px;">
					<table><tr><td><label style="margin-bottom: 0; margin-top: 2px;">' . input_radiobox_tag(
                        'payment_module',
                        $modules_id,
                        $params
                    ) . ' ' . $modules_title . '</label></td></tr></table>
		    </div>
			';
...
<script>
    function payment_module_confirmation(module_id) {
        ...
        $('#payment_module_confirmation').load('<?php echo url_for(
            'items/processes_checkout',
            'action=confirmation&id=' . _get::int('id') . '&path=' . $app_path
        ) ?>&module_id=' + module_id, function () {
