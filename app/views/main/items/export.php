...
<?php
echo ajax_modal_template_header(TEXT_HEADING_EXPORT) ?>
...
if (count($app_selected_items[$_GET['reports_id']]) == 0) {
  ...
  ' . ajax_modal_template_footer('hide-save-button');
    ...
    $fields_query = db_query(
        "select f.* from app_fields f, app_forms_tabs t where f.forms_tabs_id = t.id and f.type in ('" . implode(
            "','",
            fields_types::get_attachments_types()
        ) . "') and f.entities_id='" . db_input($current_entity_id) . "' order by t.sort_order, f.sort_order, f.name"
    );
    while ($v = db_fetch_array($fields_query)) {
        ...
        $attachments_fields[] = '<div><label>' . input_checkbox_tag(
                'fields[]',
                $v['id'],
                [
                    'id' => 'fields_' . $v['id'],
            ...
            ) . ' ' . fields_types::get_option($v['type'], 'name', $v['name']) . '</label></div>';
    }
    ?>
    ...
    <div class="modal-body">
        ...
        <ul class="nav nav-tabs" id="items_export_tabs">
            <li class="active"><a href="#select_fields_tab" data-toggle="tab"><?php
                    echo TEXT_SELECT_FIELD_TO_EXPORT ?></a></li>
            <li><a href="#my_templates_tab" data-toggle="tab"><?php
                    echo TEXT_MY_TEMPLATES ?></a></li>
        ...
        <div class="tab-content">
            <div class="tab-pane fade active in" id="select_fields_tab">
                ...
                <div id="items_export_templates_selected" style="display:none">
                    ...
                    <div class="alert alert-info">
                        ...
                        <div style="float: right"><a title="<?php
                            echo addslashes(TEXT_UPDATE_SELECTED_TEMPLATE_INFO) ?>"
                                                     href="javascript: update_items_export_templates()"><i
                                        class="fa fa-refresh" aria-hidden="true"></i> <?php
                                echo TEXT_BUTTON_UPDATE ?></a></div>
                    </div>
                </div>
                ...
                echo form_tag(
                        'export_form',
                        url_for('items/export', 'path=' . $_GET['path']),
                        ['class' => 'form-inline']
                    ) . input_hidden_tag('action', 'export') . input_hidden_tag('reports_id', $_GET['reports_id']) ?>
                <p>
                    <?php
                    ...
                    while ($tabs = db_fetch_array($tabs_query)) {
                        ...
                        $fields_query = db_query(
                            "select f.*,if(f.type in ('fieldtype_id','fieldtype_date_added','fieldtype_created_by'),-1,f.sort_order) as field_sort_order from app_fields f where  f.type not in ('fieldtype_action','fieldtype_section' {$exclude_types}) and f.entities_id='" . db_input(
                                $current_entity_id
                            ) . "' and forms_tabs_id='" . db_input($tabs['id']) . "' order by field_sort_order, f.name"
                        );
                        while ($v = db_fetch_array($fields_query)) {
                            //check field access
                            if (isset($fields_access_schema[$v['id']])) {
                            ...
                            $fileds_html .= '<div><label>' . input_checkbox_tag(
                                    'fields[]',
                                    $v['id'],
                                    [
                                        'id' => 'fields_' . $v['id'],
                        ...
                        if (strlen($fileds_html) > 0) {
                            echo '<p><div><label><b>' . input_checkbox_tag(
                                ...
                                ) . $tabs['name'] . '</b></label></div>' . $fileds_html . '</p>';
                        }
                    }
                ...
                <div class="form-group">
                    ...
                    echo select_tag('file_extension', $choices, 'xlsx', ['class' => 'form-control input-small'])
                    ?>
                </div>
            ...
            <div class="tab-pane fade" id="my_templates_tab">
                ...
                echo form_tag(
                    ...
                    url_for('items/export', 'action=save_templates&path=' . $_GET['path']),
                    ['class' => 'form-inline']
                ) ?>
                <?php
                echo TEXT_ADD_NEW_TEMPLATE ?>
                <div class="row">
                    <div class="col-md-7">
                        <?php
                        echo input_tag(
                            'templates_name',
                            '',
                            ['class' => 'form-control required', 'placeholder' => TEXT_ENTER_TEMPLATE_NAME]
                        ) ?>
                        <?php
                        echo input_hidden_tag('export_fields_list') ?>
                    </div>
                    <div class="col-md-5">
                        <?php
                        echo submit_tag(TEXT_BUTTON_ADD) ?>
                    </div>
                </div>
                </form>
                <p><?php
        ...
        ' . form_tag(
                        'export_form',
                        url_for('items/export_attachments', 'path=' . $_GET['path']),
                        ['class' => 'form-inline']
                    ) . input_hidden_tag('action', 'export') . input_hidden_tag(
                        'reports_id',
                        (int)$_GET['reports_id']
                    ) . '
    ...
    <?php
    require(component_path('items/export.js')); ?>
