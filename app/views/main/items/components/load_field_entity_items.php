        ...
        if ($field_cfg->get('entity_id') == $current_entity_id) {
            if ($app_user['group_id'] == 0) {
                $entities_query = db_query(
                    "select e.* from app_entities e where id='" . db_input(
                        $fields['entities_id']
                ...
                $entities_query = db_query(
                    "select e.* from app_entities e, app_entities_access ea where e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(
                        $app_user['group_id']
                    ) . "' and e.id = '" . db_input($fields['entities_id']) . "' order by e.sort_order, e.name"
                );
            }

            if ($entities = db_fetch_array($entities_query)) {
                if ($entity_cfg->get(
                        'item_page_field' . $fields['id'] . '_entity' . $entities['id'] . '_position'
                    ) == $field_entity_items_position) {
                    //try to get report type parent_item_info_page
                    $subentity_report_query = db_query(
                        "select * from app_reports where entities_id='" . db_input(
                            $entities['id']
                        ) . "' and reports_type='field" . $fields['id'] . "_entity_item_info_page'"
                    );
                    ...
                    if (users::has_access('create', $access_schema)) {
                        ...
                        switch (true) {
                            case $field_entity_info['parent_id'] == 0:
                                $url = url_for(
                                    'items/form',
                                    'path=' . $entities['id'] . '&fields[' . $fields['id'] . ']=' . $current_item_id . '&redirect_to=item_info_page' . $current_entity_id . '-' . $current_item_id
                                );
                                break;
                            case ($field_entity_info['parent_id'] > 0 and $field_entity_info['parent_id'] != $entity_info['parent_id']):
                                $url = url_for(
                                    'reports/prepare_add_item',
                                    'reports_id=' . $subentity_report['id'] . '&fields[' . $fields['id'] . ']=' . $current_item_id . '&redirect_to=item_info_page' . $current_entity_id . '-' . $current_item_id
                                );
                                break;
                            case ($field_entity_info['parent_id'] > 0 and $field_entity_info['parent_id'] == $entity_info['parent_id']):
                                ...
                                $url = url_for(
                                    'items/form',
                                    'path=' . $path_info['parent_entity_id'] . '-' . $path_info['parent_entity_item_id'] . '/' . $entities['id'] . '&fields[' . $fields['id'] . ']=' . $current_item_id . '&redirect_to=item_info_page' . $current_entity_id . '-' . $current_item_id
                                );
                                break;
                        }

                        $add_button = button_tag(
                                (strlen($subentity_cfg->get('insert_button')) > 0 ? $subentity_cfg->get(
                                    'insert_button'
                                ) : TEXT_ADD),
                                $url,
                                true,
                    ...
                    if (users::has_access('export_selected', $access_schema) and users::has_access(
                            'export',
                            $access_schema
                        )) {
                        $with_selected_menu .= '<li>' . link_to_modalbox(
                                ...
                                url_for(
                                    'items/export',
                                    'path=' . $subentity_report["entities_id"] . '&reports_id=' . $subentity_report['id']
                                )
                            ) . '</li>';
                    }
                    ...
                    $with_selected_menu .= plugins::include_dashboard_with_selected_menu_items(
                        $subentity_report['id'],
                        '&path=' . $app_path . '/' . $subentity_report['entities_id'] . '&redirect_to=parent_item_info_page'
                    );

                    $portlets = new portlets(
                        'entity_items_listing' . $subentity_report['id'],
                        $entity_cfg->get(
                            'item_page_field' . $fields['id'] . '_entity' . $entities['id'] . '_collapsed',
                            false
                        )
                    );
			...
			<div class="row info-page-reports-container" id="' . $listing_container . '_info_block" ' . ($entity_cfg->get(
                            'hide_item_page_field' . $fields['id'] . '_if_empty'
			    ...
			    <div class="col-md-12">		
				          ...
				          ' . (strlen(
                            $entity_cfg->get(
                                'item_page_field' . $fields['id'] . '_entity' . $entities['id'] . '_heading'
                            )
                        ) ? $entity_cfg->get(
                            'item_page_field' . $fields['id'] . '_entity' . $entities['id'] . '_heading'
                        ) : (strlen($subentity_cfg->get('listing_heading')) > 0 ? $subentity_cfg->get(
                            'listing_heading'
                        ) : $entities['name'])) . '
                        ...
                        '</div>
                            ...
                            $subentity_report['listing_order_fields']
                        ) .
                        input_hidden_tag(
                        ...
                        input_hidden_tag(
                            $listing_container . '_force_filter_by',
                            $fields['id'] . ':' . $current_item_id
                        ) .
                        input_hidden_tag(
                            $listing_container . '_redirect_to',
                            'item_info_page' . $current_entity_id . '-' . $current_item_id
                        ) . '
