...
<div class="row">
            ...
            if (users::has_comments_access('create', $access_rules->get_comments_access_schema())) {
                echo '
                    ...
                    button_tag(
                        ...
                        url_for('items/comments_form', 'path=' . $_GET['path']),
                        true,
                        ['class' => 'btn btn-primary btn-sm']
                    ) .
                    '<button onClick="quick_comment_toggle()" style="margin-left: -5px;" title="' . addslashes(
                        TEXT_QUICK_COMMENT
                    ) . '" type="button" class="btn btn-default btn-sm"><i class="fa fa-paper-plane-o" aria-hidden="true"></i></button>
      			</div>';
            }
            ?>
    ...
    <div class="col-sm-4">
        <div class="entitly-listing-buttons-right">
            <?php
            echo render_comments_search_form($entity_info['id'], 'items_comments_listing') ?>
        </div>
    </div>
</div>

<?php
require(component_path('items/quick_comment_form')); ?>
