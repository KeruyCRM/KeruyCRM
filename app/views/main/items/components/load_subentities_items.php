...
if (isset($subentities_items_position)) {
    if ($app_user['group_id'] == 0) {
        ...
        $entities_query = db_query(
            "select e.* from app_entities e, app_entities_access ea where e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(
                $app_user['group_id']
            ) . "' and e.parent_id = '" . db_input($current_entity_id) . "' order by e.sort_order, e.name"
        );
    }
    ...
    while ($entities = db_fetch_array($entities_query)) {
        if ($entity_cfg->get('item_page_subentity' . $entities['id'] . '_position') == $subentities_items_position) {
            ...
            $add_button = '';
            if (users::has_access('create', $access_schema) and access_rules::has_add_buttons_access(
                    $entities['id'],
                    $current_item_id
                )) {
                $url = url_for(
            ...
            $html = '
				...
				<div class="caption">        
                                    <a href="' . url_for(
                    'items/items',
                    'path=' . $app_path . '/' . $subentity_report['entities_id']
                ) . '">' .
                (strlen($entity_cfg->get('item_page_subentity' . $entities['id'] . '_heading')) ? $entity_cfg->get(
                    'item_page_subentity' . $entities['id'] . '_heading'
                ...
                input_hidden_tag(
                    'subentity' . $subentity_report['entities_id'] . '_items_listing_path',
                    $app_path . '/' . $subentity_report['entities_id']
                ) . ' 	      
