...
?>
    <h3 class="page-title"><?php
        echo $fields_info['name'] . ': ' . $choices_info['name'] ?></h3>

<?php
echo button_tag(
    TEXT_BUTTON_ADD_NEW_REPORT_FILTER,
    url_for(
        'entities/fields_choices_filters_form',
        'reports_id=' . $reports_info['id'] . '&choices_id=' . $_GET['choices_id'] . '&entities_id=' . $_GET['entities_id'] . '&fields_id=' . $_GET['fields_id']
    )
) ?>
                ...
                <tr>
                    <td style="white-space: nowrap;"><?php
                        echo button_icon_delete(
                                url_for(
                                    'entities/fields_choices_filters_delete',
                                    'id=' . $v['id'] . '&choices_id=' . $_GET['choices_id'] . '&reports_id=' . $reports_info['id'] . '&entities_id=' . $_GET['entities_id'] . '&fields_id=' . $_GET['fields_id']
                                )
                            )
                            . ' ' . button_icon_edit(
                                url_for(
