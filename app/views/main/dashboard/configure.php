...
<?php
echo ajax_modal_template_header(TEXT_CONFIGURE_DASHBOARD) ?>

<?php
$common_reports_query = db_query(
    "select count(*) as total from app_reports r, app_entities e, app_entities_access ea  where r.entities_id = e.id and e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(
        $app_user['group_id']
...
<?php
echo form_tag('dashboard', url_for('dashboard/', 'action=save')) ?>
...
<div class="modal-body ajax-modal-width-790">
    ...
    <ul class="nav nav-tabs" id="form_tabs">
        ...
        if (users::has_reports_access()): ?>
            ...
            <li><a data-toggle="tab" href="#form_tab_reports_header"><?php
                    echo TEXT_HEADER_TOP_MENU ?></a></li>
        <?php
        endif ?>

        <?php
        if ($common_reports_count > 0): ?>
            <li><a data-toggle="tab" href="#form_tab_common_reports"><?php
                    echo TEXT_EXT_COMMON_REPORTS ?></a></li>
        <?php
        endif ?>
    ...
    <div class="tab-content">
                            ...
                            <div class="cfg_listing">
                                <ul id="reports_on_dashboard" class="sortable sortable-reports">
                                    ...
                                    $reports_query = db_query(
                                        "select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
                                            $app_logged_users_id
                                        ) . "' and r.reports_type='standard' and r.in_dashboard=1 order by r.dashboard_sort_order, r.name"
                                    );
                            ...
                            <div class="cfg_listing">
                                <ul id="reports_excluded_from_dashboard" class="sortable sortable-reports">
                                    ...
                                    $reports_query = db_query(
                                        "select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
                                            $app_logged_users_id
                                        ) . "' and r.reports_type='standard' and r.in_dashboard!=1 order by e.name, r.name"
                                    );
                                    while ($v = db_fetch_array($reports_query)) {
                                        echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
                                    }
                            ...
                            <div class="cfg_listing">
                                <ul id="reports_counter_on_dashboard" class="sortable sortable-reports-counter">
                                    ...
                                    $reports_query = db_query(
                                        "select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
                                            $app_logged_users_id
                                        ) . "' and r.reports_type='standard' and r.in_dashboard_counter=1 order by r.dashboard_counter_sort_order, r.name"
                                    );
                                    while ($v = db_fetch_array($reports_query)) {
                                        echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
                                    }
                            ...
                            <div class="cfg_listing">
                                <ul id="reports_counter_excluded_from_dashboard"
                                    ...
                                    $reports_query = db_query(
                                        "select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
                                            $app_logged_users_id
                                        ) . "' and r.reports_type='standard' and r.in_dashboard_counter!=1 order by e.name, r.name"
                                    );
                                    while ($v = db_fetch_array($reports_query)) {
                                        echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
                                    }
        ...
        <div class="tab-pane" id="form_tab_reports_header">
            ...
            <div><?php
                echo TEXT_CONFIGURE_HOT_REPORTS_INFO ?></div>
            <br>
            ...
            <table style="width: 100%; max-width: 960px;">
                <tr>
                    <td valign="top" width="50%">
                        <fieldset>
                            <legend><?php
                                echo TEXT_DISPLAY_IN_HEADER ?></legend>
                            <div class="cfg_listing">
                                <ul id="reports_in_header" class="sortable sortable-reports-header">
                                    ...
                                    $reports_query = db_query(
                                        "select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
                                            $app_logged_users_id
                                        ) . "' and r.reports_type='standard' and r.in_header=1 order by r.header_sort_order, r.name"
                                    );
                                    while ($v = db_fetch_array($reports_query)) {
                                        echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
                                    }
                            ...
                            <div class="cfg_listing">
                                <ul id="reports_excluded_in_header" class="sortable sortable-reports-header">
                                    ...
                                    $reports_query = db_query(
                                        "select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
                                            $app_logged_users_id
                                        ) . "' and r.reports_type='standard' and r.in_header!=1 order by e.name, r.name"
                                    );
                                    while ($v = db_fetch_array($reports_query)) {
                                        echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
                                    }
        ...
        if ($common_reports_count > 0): ?>
            <div class="tab-pane" id="form_tab_common_reports">
                <p><?php
                    echo TEXT_EXT_COMMON_REPORTS_DASHBOARD_DESCRIPTION ?></p>
                <?php
                $common_reports_list = [];
                $reports_query = db_query(
...
<script>
    $(function () {
        ...
        $("ul.sortable-reports-header").sortable({
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    type: "POST",
                    url: '<?php echo url_for("dashboard/", "action=sort_reports_header")?>',
                    data: data
                });
            }
        });
    ...
    function reports_section_delete(section_id) {
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo url_for("dashboard/reports", "action=delete_section")?>',
    ...
    function reports_section_edit(section_id, type, value) {
        $.ajax({
            type: "POST",
            url: '<?php echo url_for("dashboard/reports", "action=edit_section")?>',
