...
if (strlen($reports_groups_info['counters_list'])) {
    ...
    $reports_counter->reports_query = "select r.*,e.name as entities_name,e.parent_id as entities_parent_id from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(
            $app_logged_users_id
        ) . "' and r.reports_type='standard' and  r.id in (" . $reports_groups_info['counters_list'] . ") order by field(r.id," . $reports_groups_info['counters_list'] . ")";
...
if (strlen($reports_groups_info['reports_list'])) {
    $reports_query = db_query(
        "select * from app_reports where created_by='" . db_input(
            $app_logged_users_id
        ) . "' and id in (" . $reports_groups_info['reports_list'] . ") and reports_type in ('standard') order by field(id," . $reports_groups_info['reports_list'] . ")"
    );
