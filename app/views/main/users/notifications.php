<h3 class="page-title"><?php
    echo TEXT_USERS_NOTIFICATIONS ?></h3>


<?php
echo button_tag(
    TEXT_MARK_AS_READ,
    url_for("users/notifications", 'action=delete_selected'),
    false,
...
<script>
    ...
    function handle_itmes_select(listing_container) {
        $('#' + listing_container + ' .items_checkbox').click(function () {
            $.ajax({
                type: "POST",
                url: '<?php echo url_for("users/notifications", "action=select")?>',
                data: {id: $(this).val(), checked: $(this).attr('checked')}
            });
        })
        ...
        $('#' + listing_container + ' .select_all_items').click(function () {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo url_for("users/notifications", "action=select_all")?>',
