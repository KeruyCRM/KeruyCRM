...
class Reports_sections
{
    ...
    function render()
    {
        ...
        $sections_query = db_query(
            ...
            ) . "' and created_by='" . db_input(\K::$fw->app_user['id']) . "' order by sort_order"
        );
    ...
    function get_reports_choices($sections, $type)
    {
        ...
        if (\Helpers\App::is_ext_installed()) {
            if (calendar::user_has_personal_access()) {
                $choices[\K::$fw->TEXT_EXT_CALENDAR]['calendar_personal'] = \K::$fw->TEXT_EXT_CALENDAR_PERSONAL;
            }

            if (calendar::user_has_public_access()) {
                $choices[\K::$fw->TEXT_EXT_CALENDAR]['calendar_public'] = \K::$fw->TEXT_EXT_CALENDAR_PUBLIC;
            }
            ...
            while ($v = db_fetch_array($reports_query)) {
                $choices[\K::$fw->TEXT_EXT_CALENDAR]['calendarreport' . $v['id']] = $v['name'];
            }
            ...
            while ($reports = db_fetch_array($reports_query)) {
                if (pivot_calendars::has_access($reports['users_groups'])) {
                    $choices[\K::$fw->TEXT_EXT_PIVOT_СALENDAR]['pivot_calendars' . $reports['id']] = $reports['name'];
                }
            }
            ...
            while ($v = db_fetch_array($reports_query)) {
                if (resource_timeline::has_access($v['users_groups'])) {
                    $choices[\K::$fw->TEXT_EXT_RESOURCE_TIMELINE]['resource_timeline' . $v['id']] = $v['name'];
                }
            }
        }
