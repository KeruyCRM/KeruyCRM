...
class Filters_preview
{
    ...
    static function has_default_panel_access($entity_cfg)
    {
        ...
        $panel_access = (strlen($entity_cfg->get('default_filter_panel_access')) ? explode(
            ',',
            $entity_cfg->get('default_filter_panel_access')
        ) : []);
    ...
    function count_filters()
    {
        ...
        $filters_query = db_query(
            "select count(*) as total from app_reports_filters rf, app_fields f  where rf.fields_id=f.id and rf.reports_id='" . db_input(
                $this->reports_id
            ) . "' " . (count($filters_panels_fields) ? " and f.id not in (" . implode(
                    ',',
                    $filters_panels_fields
                ) . ")" : '') . " order by rf.id",
            false
    ...
    function get_applied_filters_name()
    {
        ...
        if (isset($app_current_users_filter[$this->reports_id])) {
            return (strlen(
                $app_current_users_filter[$this->reports_id]
            ) ? $app_current_users_filter[$this->reports_id] : TEXT_APPLIED_FILTERS);
        } else {
    ...
    function render()
    {
      ...
      <div class="portlet portlet-filters-preview noprint">
      	<div class="portlet-title">
      		<div class="caption">        
            ' . $this->render_users_filters() . ' ' . $this->render_add_button(
            ) . ' ' . $this->get_applied_filters_name() . '            
          </div>' .
    ...
    function render_listing_configuration_button()
    {
      ...
      <div class="btn-group">
            ...
            ' . link_to_modalbox(
                ...
                url_for(
                    'reports/sorting',
                    'reports_id=' . $this->reports_id . '&redirect_to=' . $this->redirect_to . (strlen(
                        $this->path
                    ) > 0 ? '&path=' . $this->path : '')
                )
            ) . '
    ...
    function render_add_button()
    {
        ...
        <li>				        
          ' . link_to_modalbox(
                ...
                url_for('reports/filters_form', 'reports_id=' . $this->reports_id . $url_params)
            ) . '
  			</li>
        ';
            ...
            <li>				        
              ' . link_to_modalbox(
                        ...
                        url_for(
                            ...
                            'reports_id=' . $this->reports_id . '&parent_reports_id=' . $parent_reports_id . $url_params
                        )
                    ) . '
      			</li>
            ';
    ...
    function render_users_filters()
    {
        ...
        $url_params = '&redirect_to=' . $this->redirect_to . '&reports_id=' . $this->reports_id . (strlen(
                $this->path
            ) > 0 ? '&path=' . $this->path : '');
        ...
        <li>
  				<a href="' . url_for(
                    'reports/users_filters',
                    'action=use&id=' . $id . $url_params
                ) . '"><i class="fa fa-angle-right"></i>' . $name . '</a>
  			</li>';
        }

        //use defaulf filters for entities listing only
        $report_info = db_find('app_reports', $this->reports_id);
        if (in_array($report_info['reports_type'], ['entity_menu', 'entity'])) {
            $filters_html .= '
	      <li>
					<a href="' . url_for(
                    'reports/users_filters',
                    'action=use&id=default' . $url_params
                ) . '"><i class="fa fa-angle-right"></i>' . TEXT_DEFAULT_FILTERS . '</a>
				</li>';
        }
      ...
      <li>				        
        ' . link_to_modalbox(
                '<i class="fa fa-floppy-o"></i> ' . TEXT_SAVE_FILTERS,
                url_for('reports/users_filters_form', $url_params)
            ) . '
			</li>
    ';
        ...
        <li>				        
          ' . link_to_modalbox(
                    '<i class="fa fa-trash-o"></i> ' . TEXT_DELETE_FILTERS,
                    url_for('reports/users_filters_delete', $url_params)
                ) . '
  			</li>
    ';
        }
    ...
    function render_filters_by_report($reports_id, $is_parent = false)
    {
        ...
        $filters_query = db_query(
            "select rf.*, f.name, f.type, f.entities_id from app_reports_filters rf, app_fields f  where rf.fields_id=f.id and rf.reports_id='" . db_input(
                $reports_id
            ) . "' order by rf.id"
        );
          ...
          <div class="filters-preview-box is-active-' . $v['is_active'] . '">
          	...
          	' . ($v['is_active'] ? $this->render_filters_templates($v['fields_id'], $v['id'], $url_params) : '') . '	
                ' . $this->render_toggler($v) . '    
                    <span class="filters-preview-box-heading" onClick="open_dialog(\'' . $edit_url . '\')"  title="' . htmlspecialchars(
                    TEXT_BUTTON_EDIT
                ) . '"><b>' . fields_types::get_option(
                    $v['type'],
                    'name',
                    $v['name']
                ...
                '</span>' .
                link_to(
                    '<i class="fa fa-trash-o" title="' . htmlspecialchars(TEXT_BUTTON_REMOVE_FILTER) . '"></i>',
                    $delete_url
                ) .
                '</div>
        </li>';
        ...
        if (strlen($html) > 0) {
            ...
            <li>' .
                link_to(
                    ...
                    url_for('reports/filters', 'action=delete&id=all' . $url_params),
                    ['title' => TEXT_BUTTON_REMOVE_ALL_FILTERS, 'class' => 'btn btn-default']
                ) . ' ' .
                link_to_modalbox(
                    $entity_info['name'] . ':',
                    url_for('reports/filters_form', $url_params),
                    ['title' => TEXT_BUTTON_ADD_NEW_REPORT_FILTER, 'class' => 'btn btn-default']
                ) . '&nbsp;' .
                '</li>
            ' . $html . '
          </ul>
    ...
    function render_toggler($filter)
    {
        ...
        if (!in_array($app_module_path, ['items/items', 'reports/view'])) {
            return '';
        }
    ...
    function render_filters_templates($fields_id, $filters_id, $url_params)
    {
        ...
        $templates_query = db_query(
            "select * from app_reports_filters_templates where users_id='" . db_input(
                $app_logged_users_id
            ) . "' and fields_id='" . db_input($fields_id) . "'"
        );
        while ($templates = db_fetch_array($templates_query)) {
            if (in_array(
                $templates['filters_condition'],
                ['empty_value', 'not_empty_value', 'filter_by_overdue', 'filter_by_overdue_with_time']
            )) {
            ...
            $html_templates .= '
	  		<li class="li-templates-' . $templates['id'] . '">
	  			<a href="javascript: delete_filters_templates(' . $templates['id'] . ')" data-url="' . url_for(
                    'reports/filters',
                    'action=delete_filters_templates&templates_id=' . $templates['id'] . '&id=' . $filters_id . $url_params
                ) . '" class="action a-templates-' . $templates['id'] . '"><i class="fa fa-trash-o" title="' . addslashes(
                    TEXT_BUTTON_DELETE
                ) . '"></i></a>
  				<a href="' . url_for(
                    'reports/filters',
                    'action=use_filters_template&templates_id=' . $templates['id'] . '&id=' . $filters_id . $url_params
                ) . '">' . $fitlers_values . '</a>	  			
	  		</li>
  		';
        }
