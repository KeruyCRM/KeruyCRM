        ...
        while ($user = db_fetch_array($users_query)) {
            ...
            $reports_query = db_query(
                "select r.* from app_reports r, app_entities e, app_entities_access ea  where r.entities_id = e.id and e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(
                    $app_user['group_id']
                ) . "' and find_in_set(" . $app_user['group_id'] . ",r.users_groups) and r.reports_type = 'common' " . $where_sql . " order by r.dashboard_sort_order, r.name"
            );
            ...
            $reports_query = db_query(
                "select * from app_reports where ((created_by='" . $app_user['id'] . "' and reports_type='standard') " . (count(
                    $common_reports_list
