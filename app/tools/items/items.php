...
class Items
{
    ...
    public static function get_choices($entity_id, $add_empty = false, $empty_text = '')
    {
        ...
        $listing_sql_query = self::add_access_query($entity_id, $listing_sql_query);

        //include access to parent records
        $listing_sql_query .= self::add_access_query_for_parent_entities($entity_id);

        $listing_sql_query .= self::add_listing_order_query_by_entity_id($entity_id);
        ...
        while ($item = db_fetch_array($items_query)) {
            $path_info = self::get_path_info($entity_id, $item['id']);
    ...
    public static function get_choices_by_entity($entity_id, $parent_entity_id, $add_empty = false)
    {
        ...
        //check view assigned only access
        $listing_sql_query = self::add_access_query($parent_entity_id, $listing_sql_query);

        //include access to parent records
        $listing_sql_query .= self::add_access_query_for_parent_entities($parent_entity_id);

        $listing_sql_query .= self::add_listing_order_query_by_entity_id($parent_entity_id);
        ...
        while ($item = db_fetch_array($items_query)) {
            $path_info = self::get_path_info($parent_entity_id, $item['id']);
    ...
    public static function get_fields_values_cache($fields_cache, $path_array, $current_entity_id)
    {
        foreach ($path_array as $v) {
            ...
            /*$fields_query = \K::model()->db_query_exec(
                'select f.* from app_fields f where f.type not in (' . \Models\Main\Fields_types::get_reserverd_types_list(
                ) . ') and  f.entities_id = ? order by f.sort_order, f.name',
                $entity_id
            );*/
    ...
    public static function get_sub_entities_list_by_path($path)
    {
        ...
        $parent_id = self::get_paretn_entity_id_by_path($path);
    ...
    public static function add_access_query_for_parent_entities($entities_id, $listing_sql_query = '')
    {
        ...
        if ($entity_info['parent_id'] > 0) {
            $listing_sql_query = ' and e.parent_item_id in (select e.id from app_entity_' . $entity_info['parent_id'] . ' e where e.id>0 ' . self::add_access_query(
    ...
    public static function add_access_query_for_user_parent_entities($entities_id, $listing_sql_query = '')
    {
        ...
        if ($entity_info['parent_id'] > 0) {
            $listing_sql_query = ' and e.parent_item_id in (select e.id from app_entity_' . $entity_info['parent_id'] . ' e where ' . ($entity_info['parent_id'] == 1 ? 'e.id=' . $app_user['id'] : 'e.id>0') . ' ' . self::add_access_query_for_user_parent_entities(
                    $entity_info['parent_id']
                ) . ')';
        }
    ...
    public static function get_paretn_users_list($entities_id, $parent_entity_item_id)
    {
