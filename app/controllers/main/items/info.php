...
class BLANK extends \Controller
{
    public function __construct()
    {
        ...
        if ($app_redirect_to == 'subentity' and $entity_cfg->get(
                'redirect_after_click_heading',
                'subentity'
            ) == 'subentity') {
            $entity_query = db_query(
                "select id from app_entities where parent_id='" . db_input(
                    $current_entity_id
    ...
    public function preview_user_photo()
    {
    ...
    public function download_user_photo()
    {
    ...
    public function preview_attachment_image()
    {
        $file = attachments::parse_filename(base64_decode($_GET['file']));
    ...
    public function download_all_attachments()
    {
        $item_info = db_find('app_entity_' . $current_entity_id, $current_item_id);
