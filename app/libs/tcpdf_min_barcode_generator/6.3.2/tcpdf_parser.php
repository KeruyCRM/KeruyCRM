	...
	protected function decodeXref($startxref, $xref=array()) {
		...
		while (preg_match('/([0-9]+)[\x20]([0-9]+)[\x20]?([nf]?)(\r\n|[\x20]?[\r\n])/', $this->pdfdata, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
			...
			if ($matches[3][0] == 'n') {
				...
				$index = $obj_num. 'K' .intval($matches[2][0]);
		...
		if (preg_match('/trailer[\s]*<<(.*)>>/isU', $this->pdfdata, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
			...
			if (!isset($xref['trailer']) OR empty($xref['trailer'])) {
				...
				if (preg_match('/Root[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) {
					$xref['trailer']['root'] = intval($matches[1]). 'K' .intval($matches[2]);
				}
				if (preg_match('/Encrypt[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) {
					$xref['trailer']['encrypt'] = intval($matches[1]). 'K' .intval($matches[2]);
				}
				if (preg_match('/Info[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', $trailer_data, $matches) > 0) {
					$xref['trailer']['info'] = intval($matches[1]). 'K' .intval($matches[2]);
				}
		...
		switch ($char) {
			...
			default: {
				if (substr($this->pdfdata, $offset, 6) == 'endobj') {
					...
					$objtype = 'objref';
					$offset += strlen($matches[0]);
					$objval = intval($matches[1]). 'K' .intval($matches[2]);
