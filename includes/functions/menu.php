...
function build_user_menu()
{
    ...
    if (guest_login::is_guest()) {
        if (strlen($app_user['multiple_access_groups'])) {
            $menu[] = [
                'title' => TEXT_CHANGE_ACCESS_GROUP,
        ...
        $menu[] = ['title' => TEXT_LOGOFF, 'url' => url_for('users/login&action=logoff'), 'class' => 'fa-sign-out'];

        return $menu;
    }

    //build menu for standard tuser
    $menu[] = ['title' => TEXT_MY_ACCOUNT, 'url' => url_for('users/account'), 'class' => 'fa-user'];

    if (is_ext_installed() and CFG_LOGIN_DIGITAL_SIGNATURE_MODULE > 0) {
        $menu[] = [
            'title' => TEXT_EXT_MY_DIGITAL_SIGNATURE,
    ...
    $menu[] = [
        'title' => TEXT_CONFIGURE_DASHBOARD,
        'url' => url_for('dashboard/configure'),
        'modalbox' => true,
        'class' => 'fa-bars'
    ];
    $menu[] = [
        'title' => TEXT_CONFIGURE_THEME,
        'url' => url_for('dashboard/configure_theme'),
        'modalbox' => true,
        'class' => 'fa-gear'
    ];

    if ((!in_array($app_user['group_id'], explode(',', CFG_APP_DISABLE_CHANGE_PWD)) or strlen(
                CFG_APP_DISABLE_CHANGE_PWD
            ) == 0) and CFG_USE_LDAP_LOGIN_ONLY == false) {
        $menu[] = [
...
function build_main_menu()
{
    ...
    if ($app_user['group_id'] == 0) {
        ...
        $ss[] = ['title' => TEXT_MENU_USER_REGISTRATION_EMAIL, 'url' => url_for('configuration/users_registration')];
...
function renderSidebarMenu($menu = [], $html = '', $level = 0)
{
    if ($level > 0) {
        $html .= '
        <ul class="sub-menu">';
...
function renderDropDownMenu($menu = [], $html = '', $level = 0)
{
    if ($level == 0) {
        $html .= '
...
function renderNavbarMenu($menu = [], $html = '', $level = 0, $selected_id = 0)
{
    if (strlen($html) == 0) {
