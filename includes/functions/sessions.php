...
if (STORE_SESSIONS == 'mysql') {
    ...
    function _sess_read($key)
    {
        $value_query = db_query(
            "select value from app_sessions where sesskey = '" . db_input($key) . "' and expiry > '" . time() . "'"
        );
    ...
    function _sess_write($key, $val)
    {
        ...
        if ($check['total'] > 0) {
            return db_query(
                "update app_sessions set expiry = '" . db_input($expiry) . "', value = '" . db_input(
                    $value
                ) . "' where sesskey = '" . db_input($key) . "'"
            );
        } else {
            return db_query(
                "insert into app_sessions values ('" . db_input($key) . "', '" . db_input($expiry) . "', '" . db_input(
                    $value
                ) . "')"
            );
        }
