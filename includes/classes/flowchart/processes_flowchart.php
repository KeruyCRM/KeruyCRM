...
class processes_flowchart
{
    ...
    function prepare_data()
    {
        ...
        while ($process = db_fetch_array($processes_query)) {
            ...
            //process node
            $this->nodes[] = "{ data: { id: 'process_{$process['id']}',name: '" . addslashes(
                    $process['entities_name'] . ': ' . $process['name']
                ) . "'}, classes:'process', position: { x: {$this->x}, y: {$this->y} }}";
    ...
    function prepare_process_filter_node($process)
    {
        ...
        if ($reports_info = db_fetch_array($reports_info_query)) {
            ...
            if (strlen($filters_title)) {
                ...
                $this->nodes[] = "{ data: { id: 'process_filter_{$process['id']}',name: '" . addslashes(
                        $filters_title
                    ) . "'}, classes:'process_filter', position: { x: {$this->x}, y: {$this->y} }}";
    ...
    function prepare_process_actions_node($process)
    {
        ...
        $actions_query = db_query(
            "select pa.*, p.name as process_name from app_ext_processes_actions pa, app_ext_processes p where pa.process_id='" . $process['id'] . "' and  p.id=pa.process_id order by pa.sort_order"
        );
        while ($actions = db_fetch_array($actions_query)) {
            $this->x = $actions_x;

            $this->nodes[] = "{ data: { id: 'action_{$actions['id']}',process_id:{$actions['process_id']}, name: '" . addslashes(
                    $actions_types[$actions['type']]
                ) . "'}, classes:'actions', position: { x: {$this->x}, y: {$this->y} }}";
    ...
    function prepare_process_actions_edge($process, $actions, $previos_action_id)
    {
        if (!$previos_action_id) {
            if (isset($this->process_has_filter[$process['id']])) {
                ...
                $this->edges[] = "{ data: { id: 'process_action_yes_{$actions['id']}', source: 'process_filter_{$process['id']}', target: 'action_{$actions['id']}',label: '" . addslashes(
                        TEXT_YES
                    ) . "'} }";
            } else {
    ...
    function prepare_process_actions_filter_node($actions)
    {
        ...
        $reports_info_query = db_query(
            "select * from app_reports where entities_id='" . db_input(
                $action_entity_id
            ) . "' and reports_type='process_action" . $actions['id'] . "'"
        );
        if ($reports_info = db_fetch_array($reports_info_query)) {
            $filters_title = '';
            $filters_query = db_query(
            ...
            if (strlen($filters_title)) {
                ...
                $this->nodes[] = "{ data: { id: 'action_filter_{$actions['id']}', process_id:{$actions['process_id']}, name: '" . addslashes(
                        $filters_title
                    ) . "'}, classes:'process_filter', position: { x: {$this->x}, y: {$this->y} }}";
    ...
    function prepare_process_actions_fields_node($actions)
    {
        ...
        while ($actions_fields = db_fetch_array($actions_fields_query)) {
            ...
            $this->nodes[] = "{ data: { id: 'field_{$actions_fields['id']}',name: '" . addslashes(
                    $title
                ) . "'}, classes:'field', position: { x: {$this->x}, y: {$this->y} }}";
    ...
    function prepare_process_actions_fields_edge($actions, $actions_fields, $previos_field_id)
    {
        if (!$previos_field_id) {
            if (isset($this->action_has_filter[$actions['id']])) {
                ...
                $this->edges[] = "{ data: { id: 'action_filed_yes_{$actions_fields['id']}', source: 'action_filter_{$actions['id']}', target: 'field_{$actions_fields['id']}',label: '" . addslashes(
                        TEXT_YES
                    ) . "'} }";
            } else {
