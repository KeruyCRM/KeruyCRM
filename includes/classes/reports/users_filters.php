...
class users_filters
{
    ...
    function use_reports_filters($filters_id, $reports_id)
    {
        ...
        $filters_query = db_query(
            "select * from app_user_filters_values where filters_id='" . db_input(
                $filters_id
            ) . "' and reports_id='" . db_input($reports_id) . "'"
        );
        while ($filters = db_fetch_array($filters_query)) {
            $sql_data = [
                'reports_id' => $reports_id,
    ...
    function use_default_filters()
    {
        ...
        if ($reports_info = db_fetch_array($reports_info_query)) {
            $filters_query = db_query(
                "select * from app_reports_filters where reports_id='" . db_input($reports_info['id']) . "'"
            );
            while ($filters = db_fetch_array($filters_query)) {
                $sql_data = [
    ...
    function set_reports_settings($filters_id)
    {
        ...
        if ($filters_id == 'default') {
            ...
            if ($reports_info = db_fetch_array($reports_info_query)) {
                db_query(
                    "update app_reports set fields_in_listing='" . db_input(
                        $reports_info['fields_in_listing']
                    ) . "', listing_order_fields='" . db_input(
                        $reports_info['listing_order_fields']
                    ) . "'  where id='" . db_input($this->reports_id) . "'"
                );
            }
        } else {
            ...
            if (strlen($filters_info['fields_in_listing'])) {
                db_query(
                    "update app_reports set fields_in_listing='" . db_input(
                        $filters_info['fields_in_listing']
                    ) . "' where id='" . db_input($this->reports_id) . "'"
                );
            }
            ...
            if (strlen($filters_info['listing_order_fields'])) {
                db_query(
                    "update app_reports set listing_order_fields='" . db_input(
                        $filters_info['listing_order_fields']
                    ) . "' where id='" . db_input($this->reports_id) . "'"
                );
            }
        }
