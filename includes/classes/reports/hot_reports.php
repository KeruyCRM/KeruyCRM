...
class hot_reports
{
    ...
    function render()
    {
        ...
        <script>
          ...
          {
            $("#hot_reports_' . $reports['id'] . '").load("' . url_for(
                    "dashboard/",
                    "action=update_hot_reports&reports_id=" . $reports['id']
                ) . '",function(){
    ...
    function get_items($report_info, $options = [])
    {
        ...
        $listing_sql_query_select = fieldtype_formula::prepare_query_select(
            ...
            $listing_sql_query_select,
            false,
            ['reports_id' => $report_info['id'], 'fields_in_listing' => $field_heading_id]
        );
        ...
        while ($item = db_fetch_array($items_query)) {
            ...
            if (strlen($path_info['parent_name']) > 0) {
                ...
                if (isset($options['is_email'])) {
                    $parent_name = '<span style="color: #9A9A9A;"> &laquo; ' . implode(
                            ' &laquo; ',
                            $parent_name_array
                        ) . '</span>';
                } else {
            ...
            $items_array[] = [
                ...
                'name' => ($field_heading_id ? items::get_heading_field_value(
                        $field_heading_id,
                        $item
                    ) : $item['id']) . $parent_name
            ];
    ...
    function render_dropdown($id)
    {
        ...
        if ($report_info = db_fetch_array($report_info_query)) {
	        ...
	        ';
                ...
                $dropdown_menu_height = (count($items_info['items_array']) < 11 ? (count(
                        $items_info['items_array']
                    ) * 42 + 42) : 420);

                $badge_html = ($items_info['items_count'] > 0 ? '<span class="badge badge-warning">' . $items_info['items_count'] . '</span>' : '');

                $menu_icon = (strlen($report_info['menu_icon']) ? $report_info['menu_icon'] : (strlen(
                    $entity_cfg['menu_icon']
                ) > 0 ? $entity_cfg['menu_icon'] : 'fa-reorder'));
    ...
    function reports_query()
    {
        ...
        $reports_query = "select r.*,e.name as entities_name,e.parent_id as entities_parent_id from app_reports r, app_entities e where e.id=r.entities_id and ((r.created_by='" . db_input(
                ...
                $common_reports_list
                    ...
                    ',',
                    $common_reports_list
                ) . ")" : "") . ") order by r.header_sort_order, r.dashboard_sort_order, r.name";
