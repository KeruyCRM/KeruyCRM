...
class fieldtype_user_photo
{
    ...
    {
        $this->options = ['name' => TEXT_FIELDTYPE_USER_PHOTO_TITLE, 'title' => TEXT_FIELDTYPE_USER_PHOTO_TITLE];
    }
    ...
    function render($field, $obj, $params = [])
    {
        ...
        if (strlen($filename) > 0) {
            ...
            $html .= image_tag(DIR_WS_USERS . $file['file_sha1'], ['class' => 'user-photo-in-form']) . input_hidden_tag(
                    'user_photo',
                    $filename
                );
        }
        ...
        if (IS_AJAX or $app_module_path == 'users/registration') {
            $html .= '<a href="javascript: open_sub_dialog(\'' . url_for(
                    'users/photo'
                ) . '\')" class="btn btn-sm btn-default">' . TEXT_UPLOAD . '</a>';
        } else {
    ...
    function output($options)
    {
        if (strlen($options['value']) > 0) {
            ...
            ) ? DIR_WS_USERS . $file['file_sha1'] : 'images/no_photo.png');
            ...
            if (isset($options['is_print'])) {
                ...
                return image_tag(DIR_WS_USERS . $file['file_sha1'], ['width' => 50]);
            } else {
                return '
        		<div class="attachments-gallery">
                    ...
                    ) . '">' . image_tag(DIR_WS_USERS . $file['file_sha1']) . '</a></div>
    ...
    static function prepare_filename($entity_id, $item)
    {
        ...
        if (strlen($item['field_10']) and strstr($item['field_10'], 'tmp_photo')) {
            $pathinfo = pathinfo(DIR_WS_USERS . $item['field_10']);
            $new_name = 'user_' . $item['id'] . '.' . $pathinfo['extension'];
            rename(DIR_WS_USERS . $item['field_10'], DIR_WS_USERS . $new_name);
