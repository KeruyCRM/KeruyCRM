...
class fieldtype_google_map_directions
{
    ...
    function get_configuration()
    {
        ...
        $cfg[TEXT_SETTINGS][] = [
            ...
            'name' => 'api_key',
            'type' => 'input',
            'tooltip' => TEXT_FIELDTYPE_GOOGLE_MAP_API_KEY_TIP,
            'params' => ['class' => 'form-control input-xlarge required']
        ];

        $cfg[TEXT_SETTINGS][] = [
            'title' => TEXT_ADDRESS . fields::get_available_fields_helper(
                    $_POST['entities_id'],
        ...
        $cfg[TEXT_SETTINGS][] = [
            ...
            'name' => 'map_height',
            'type' => 'input',
            'tooltip_icon' => TEXT_HEIGHT_INPUT_TIP,
            'params' => ['class' => 'form-control input-small']
        ];
        ...
        $cfg[TEXT_DIRECTIONS][] = [
            'title' => TEXT_OPTIMIZE_WAYPOINTS,
            'name' => 'optimizeWaypoints',
            'type' => 'checkbox'
        ];
        $cfg[TEXT_DIRECTIONS][] = [
            'title' => TEXT_PROVIDE_ROUTE_ALTERNATIVES,
            'name' => 'provideRouteAlternatives',
            'type' => 'checkbox'
        ];
        $cfg[TEXT_DIRECTIONS][] = ['title' => TEXT_AVOID_FERRIES, 'name' => 'avoidFerries', 'type' => 'checkbox'];
        $cfg[TEXT_DIRECTIONS][] = ['title' => TEXT_AVOID_HIGHWAYS, 'name' => 'avoidHighways', 'type' => 'checkbox'];
        $cfg[TEXT_DIRECTIONS][] = ['title' => TEXT_AVOID_TOLLS, 'name' => 'avoidTolls', 'type' => 'checkbox'];

        $cfg[TEXT_DISTANCE][] = [
            'title' => TEXT_CALCULATE_TOTAL_DISTANCE,
        ...
        $cfg[TEXT_DISTANCE][] = [
            ...
            'name' => 'unit_system',
            'type' => 'dropdown',
            'choices' => ['kilometers' => TEXT_KILOMETERS, 'miles' => TEXT_MILES],
            'params' => ['class' => 'form-control input-medium']
        ];
		...
		<div id="goolge_map_container' . $field_id . '" style="width:100%; max-width: ' . $map_width . '; height: ' . $map_height . ';"></div> 
            ...
            <script>
                function calculateDistance(legs)
                {
                    ...
                    'unit_system'
                ) == 'kilometers' ? TEXT_TOTAL_DISTANCE_IN_KILOMETERS : TEXT_TOTAL_DISTANCE_IN_MILES) . ': "+distance+"</div>")
                    ...
                    foreach ($address_pattern_array as $address_key => $address_pattern) {
                        ...
                        if ((!strlen($lat) or $use_address != $current_address) and strlen($use_address)) {
                            ...
                            if (isset($result['error_message'])) {
                                $alerts->add(
                                    TEXT_FIELD . ' "' . $fields['name'] . '": ' . $result['error_message'],
                                    'error'
                                );
                            } else {
