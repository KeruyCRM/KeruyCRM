...
class fieldtype_input_datetime
{
    ...
    {
        $this->options = ['title' => TEXT_FIELDTYPE_INPUT_DATETIME_TITLE];
    }
    ...
    function get_configuration()
    {
        ...
        $cfg[TEXT_SETTINGS][] = [
            'title' => TEXT_DATE_FORMAT_IN_CALENDAR,
    ...
    function render($field, $obj, $params = [])
    {
        ...
        $attributes = [
            'class' => 'form-control fieldtype_input_datetime field_' . $field['id'] . ($field['is_required'] == 1 ? ' required' : '') . ($cfg->get(
                    'is_unique'
                ) > 0 ? ' is-unique' : '')
        ];
