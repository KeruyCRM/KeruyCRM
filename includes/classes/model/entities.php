...
class entities
{
    ...
    public static function get_flowchart_shcema($parent_id = 0, $tree = [], $level = 0, $x = 0, $y = 0)
    {
        ...
        while ($entities = db_fetch_array($entities_query)) {
            ...
            $check_fields_query = db_query(
                "select * from app_fields where entities_id = '" . $entities['id'] . "' and type in ('fieldtype_users','fieldtype_users_ajax','fieldtype_grouped_users','fieldtype_entity','fieldtype_related_records','fieldtype_formula')"
            );
    ...
    {
        $sql_data = ['name' => TEXT_INFO, 'entities_id' => $id];
            ...
            if (count($relationship) > 0) {
                if (!defined(TEXT_WARN_DELETE_ENTITY_HAS_RELATIONSHIP)) {
                    define(
                        ...
                        'You can\'t delete entity <b>%s</b> because it has relationship with: <br>%s.<br><br>Delete all fields which related to this entity.'
                    );
                }
    ...
    public static function get_tree(
        ...
        if ($app_user['group_id'] == 0) {
            ...
            $entities_query = db_query(
                "select e.* from app_entities e, app_entities_access ea where e.parent_id='" . $parent_id . "' and e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(
                    $app_user['group_id']
