...
class attachments
{
            ...
            if ($cfg->get('allow_change_file_name') and in_array(
                    ...
                    if ($has_delete_access) {
                        $html .= '
                            <span class="input-group-addon">
                                ...
                                'confirm_delation'
                            ) . '" data-filename="' . $file['file'] . '" data-row-id="' . $row_id . '" title="' . TEXT_DELETE . '"></i>
                            </span>        
                            ';
                    }
                ...
                foreach ($attachments_list as $k => $v) {
                    ...
                    if ($has_delete_access) {
                        $html .= '
                            <tr class="' . $row_id . '">
                              <td width="100%">' . app_crop_str(
                                $file['name']
                            ) . '<small>' . fieldtype_attachments::add_file_date_added($file, $cfg) . '</small></td>
                              <td align="right"><label class="checkbox delete_attachments">
                                          <i class="fa fa-trash-o pointer delete_attachments_checkbox" data-confirm-delation="' . $cfg->get(
                                'confirm_delation'
                            ) . '" data-filename="' . $file['file'] . '" data-row-id="' . $row_id . '" title="' . TEXT_DELETE . '"></i></label>
                              </td>
                            </tr>
                          ';
                    } else {
                        $html .= '
                            <tr class="' . $row_id . '">
                              <td width="100%" style="padding-bottom: 5px;">' . $file['name'] . '<small>' . fieldtype_attachments::add_file_date_added(
                                $file,
                                $cfg
                            ) . '</small></td>	           
                            </tr>
    ...
    static function get_image_preview_filepath($file)
    {
        ...
        //get foler
        $filepath = DIR_WS_ATTACHMENTS_PREVIEW . date('Y', $filetime) . '/' . date('m', $filetime) . '/' . date(
                'd',
                $filetime
            ) . '/' . $file['file_sha1'];
    ...
    static function prepare_image_preview($file)
    {
        ...
        if (!is_dir(
            DIR_WS_ATTACHMENTS_PREVIEW . date('Y', $filetime) . '/' . date('m', $filetime) . '/' . date('d', $filetime)
        )) {
            mkdir(
                DIR_WS_ATTACHMENTS_PREVIEW . date('Y', $filetime) . '/' . date('m', $filetime) . '/' . date(
                    'd',
                    $filetime
                )
            );
