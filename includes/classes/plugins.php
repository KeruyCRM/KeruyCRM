...
class plugins
{
    ...
    static public function include_dashboard_with_selected_menu_items($reports_id, $url_params = '')
    {
        ...
        if (is_ext_installed()) {
            if (users::has_access('update_selected', $access_schema)) {
                ...
                $html .= '<li>' . link_to_modalbox(
                        '<i class="fa fa-edit"></i> ' . TEXT_EXT_UPDATE_RECORDS,
                        url_for('ext/with_selected/update', 'reports_id=' . $reports_id . $url_params)
                    ) . '</li>';
                ...
                if (count(
                    ...
                    $html .= '<li>' . link_to_modalbox(
                            '<i class="fa fa-link"></i> ' . TEXT_EXT_LINK_RECORDS,
                            url_for(
            ...
            if (users::has_access('copy', $access_schema)) {
                $html .= '<li>' . link_to_modalbox(
                        '<i class="fa fa-files-o"></i> ' . TEXT_COPY_RECORDS,
                        url_for('ext/with_selected/copy', 'reports_id=' . $reports_id . $url_params)
                    ) . '</li>';
            }
            ...
            if (users::has_access('move', $access_schema)) {
                ...
                if ($entity_info['parent_id'] > 0) {
                    $html .= '<li>' . link_to_modalbox(
                            '<i class="fa fa-arrows-h"></i> ' . TEXT_MOVE_RECORDS,
                            url_for('ext/with_selected/move', 'reports_id=' . $reports_id . $url_params)
                        ) . '</li>';
                }
            }
        ...
        if (entities::has_subentities($reports_info['entities_id']) == 0 and users::has_access(
                'delete',
                $access_schema
            ) and users::has_access('delete_selected', $access_schema) and $reports_info['entities_id'] != 1) {
            $html .= '<li>' . link_to_modalbox(
