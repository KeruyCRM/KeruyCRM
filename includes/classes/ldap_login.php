...
class ldap_login
{
    ...
    function do_ldap_login($username, $password)
    {
        if (!@extension_loaded('ldap')) {
            return ['status' => false, 'msg' => TEXT_LDAP_ERROR_NOT_AVAILABLE];
        }
        ...
        if (!$ldap) {
            return ['status' => false, 'msg' => TEXT_LDAP_ERROR_CONNECTION];
        }
        ...
        if (strlen($this->config['ldap_user']) > 0 || strlen($this->config['ldap_password']) > 0) {
            if (!@ldap_bind(
                $ldap,
                htmlspecialchars_decode($this->config['ldap_user']),
                htmlspecialchars_decode($this->config['ldap_password'])
            )) {
                return ['status' => false, 'msg' => TEXT_LDAP_ERROR_BINDING];
            }
        }

        // ldap_connect only checks whether the specified server is valid, so the connection might still fail
        $search_filters = ['cn', htmlspecialchars_decode($this->config['ldap_uid'])];
        ...
        if (is_array($ldap_result) && sizeof($ldap_result) > 1) {
            if (@ldap_bind($ldap, $ldap_result[0]['dn'], htmlspecialchars_decode($password))) {
                ...
                return ['status' => false, 'msg' => TEXT_LDAP_ERROR_INCORRECT_PASSWORD];
            }
        }

        @ldap_close($ldap);

        return ['status' => false, 'msg' => TEXT_LDAP_ERROR_INCORRECT_USERNAME];
    }
    ...
    function ldap_user_filter($username)
    {
        ...
        if ($this->config['ldap_user_filter']) {
            $_filter = ($this->config['ldap_user_filter'][0] == '(' && substr(
                    $this->config['ldap_user_filter'],
                    -1
                ) == ')') ? $this->config['ldap_user_filter'] : "({$this->config['ldap_user_filter']})";
    ...
    function ldap_get_user_group($ldap, $username)
    {
        ...
        while ($groups = db_fetch_array($groups_query)) {
            $filter = "(&(" . htmlspecialchars_decode(
                    $this->config['ldap_uid']
                ) . "=" . $username . ")" . $groups['ldap_filter'] . ")";
