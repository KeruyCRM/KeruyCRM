 ...
 * 
 * 
 * @flexphperia - deleted some unused methods
 */
...
class Flexphperia_GdThumb
{
     ...
     *
     * Also sets $this->hasError to true, so even if the exceptions are caught,
     * we don't
     ...
     *
     * If either param is set to zero, then that dimension will not be
     * considered as a part of the resize.
     * Additionally, if $this->options['resizeUp'] is set to true (false by
     * default), then this function will
    ...
    public function resize($maxWidth = 0, $maxHeight = 0)
    {
        ...
        if (function_exists('imagecreatetruecolor')) {
            $this->workingImage = imagecreatetruecolor($this->newDimensions['newWidth'], 
        ...
        imagecopyresampled($this->workingImage, $this->oldImage, 0, 0, 0, 0, $this->newDimensions['newWidth'], 
            $this->newDimensions['newHeight'], $this->currentDimensions['width'], $this->currentDimensions['height']);
     ...
     * @return GdThumb
     */
    public function crop($startX, $startY, $cropWidth, $cropHeight, $squareSize = false)
    {
        // validate input
        ...
        //for leaflet we dont neeed to ensure that werer in bounds
        ...
        // create the working image
        if (function_exists('imagecreatetruecolor')) {
            $this->workingImage = imagecreatetruecolor(256, 256);
        } else {
            $this->workingImage = imagecreate(256, 256);
        }
		
		// create the working image
		if (function_exists('imagecreatetruecolor')) {
			$this->workingImage = imagecreatetruecolor($squareSize ? $squareSize : $cropWidth, $squareSize ? $squareSize : $cropHeight);
		} else {
			$this->workingImage = imagecreate($squareSize ? $squareSize : ($cropHeight ? $squareSize : $cropHeight));
		}
        
        $this->preserveAlpha();
        
        imagecopyresampled($this->workingImage, $this->oldImage, 0, 0, $startX, $startY, $cropWidth, $cropHeight, 
     ...
     * Saves an image
     *
     * This function will make sure the target directory is writeable, and then
     * save the image.
     *
     * If the target directory is not writeable, the function will try to
     * correct the permissions (if allowed, this
     * is set as an option ($this->options['correctPermissions']). If the target
     * cannot be made writeable, then a
     ...
     * @param string $format
     *            The format to save the image in (optional, must be one of
     *            [GIF,JPG,PNG]
     * @return GdThumb
     */
    ...
    /**
     * Calculates a new width and height for the image based on $this->maxWidth
     * and the provided dimensions
     *
     * @return array
     * @param int $width            
     * @param int $height            
    ...
    /**
     * Calculates a new width and height for the image based on $this->percent
     * and the provided dimensions
     *
     * @return array
     * @param int $width            
     ...
     *
     * These calculations are based on both the provided dimensions and
     * $this->maxWidth and $this->maxHeight
     *
     * @param int $width            
     * @param int $height            
     */
    ...
    /**
     * Calculates new image dimensions, not allowing the width and height to be
     * less than either the max width or height
     *
     * @param int $width            
     * @param int $height            
     */
    ...
    /**
     * Calculates new dimensions based on $this->percent and the provided
     * dimensions
     *
     * @param int $width            
     * @param int $height            
     ...
     *
     * Alpha / transparency will not be preserved if the appropriate options are
     * set to false.
     * Also, the GIF transparency is pretty skunky (the results aren't awesome),
     * but it works like a
     * champ... that's the nature of GIFs tho, so no huge surprise.
     *
     * This functionality was originally suggested by commenter Aimi (no links /
     * site provided) - Thanks! :)
     */
    protected function preserveAlpha()
    {
        if ($this->options['outputFormat'] == 'png' && $this->options['preserveAlpha'] === true) {
        
            imagealphablending($this->workingImage, false);
            
            $colorTransparent = imagecolorallocatealpha($this->workingImage, $this->options['alphaMaskColor'][0], 
                $this->options['alphaMaskColor'][1], $this->options['alphaMaskColor'][2], 127);
        ...
        if ($this->format == 'GIF' && $this->options['preserveTransparency'] === true) {
            $colorTransparent = imagecolorallocate($this->workingImage, $this->options['transparencyMaskColor'][0], 
                $this->options['transparencyMaskColor'][1], $this->options['transparencyMaskColor'][2]);
