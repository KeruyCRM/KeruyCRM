...
if (strlen($reports_info['fields_in_listing']) > 0) {
    ...
    $listing_types_query = db_query(
        "select settings from app_listing_types where  type='" . $reports_info['listing_type'] . "' and entities_id='" . $reports_info['entities_id'] . "'"
    );
    ...
    if (is_array($settings->get('fields_in_listing'))) {
        ...
        $fields_query = db_query(
            "select f.* from app_fields f where f.listing_status=1 order by f.listing_sort_order, f.name"
        );
...
switch ($app_module_action) {
    ...
    case 'set_rows_per_page':
        ...
        db_query(
            "update app_reports set rows_per_page='" . db_input($_POST['rows_per_page']) . "' where id='" . db_input(
                $_GET['reports_id']
            ) . "'"
        );
        exit();
