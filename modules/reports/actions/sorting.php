...
switch ($app_module_action) {
        ...
        if (strlen($_POST['fields_for_sorting']) > 0) {
            foreach (explode(',', str_replace('form_fields_', '', $_POST['fields_for_sorting'])) as $v) {
                if (strstr($v, 'lastcommentdate')) {
                    $listing_order_fields = array_merge([$v], $listing_order_fields);
                } else {
        ...
        if (count($listing_order_fields) > 0) {
            db_query(
                "update app_reports set listing_order_fields='" . db_input(
                    implode(',', $listing_order_fields)
                ) . "' where id='" . db_input($_GET['reports_id']) . "'"
            );
        } else {
    ...
    case 'set_sorting_condition':
        if ($_POST['condition'] == 'desc') {
            db_query(
                "update app_reports set listing_order_fields=replace(listing_order_fields,'" . $_POST['field_id'] . "_asc','" . $_POST['field_id'] . "_desc') where id='" . db_input(
                    $_GET['reports_id']
                ) . "'"
            );
        } else {
            db_query(
                "update app_reports set listing_order_fields=replace(listing_order_fields,'" . $_POST['field_id'] . "_desc','" . $_POST['field_id'] . "_asc') where id='" . db_input(
                    $_GET['reports_id']
                ) . "'"
            );
        }
