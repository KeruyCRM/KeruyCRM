...
if ($app_redirect_to == 'listng_filters') {
    ...
    echo form_tag(
        ...
        url_for(
            ...
            'entities_id=' . $fields_info['entities_id'] . '&fields_id=' . $fields_id
        )
    );
} elseif ($app_redirect_to == 'related_records_field_settings') {
    echo form_tag(
        'sorting_form',
        url_for('entities/fields_settings', 'entities_id=' . $_GET['entities_id'] . '&fields_id=' . $_GET['fields_id'])
    );
} elseif ($app_redirect_to == 'parent_infopage_filters') {
    echo form_tag(
        'sorting_form',
        url_for('entities/parent_infopage_filters', 'entities_id=' . $reports_info['entities_id'])
    );
    ...
    echo form_tag(
        ...
        url_for(
            ...
            'entities_id=' . $reports_info['entities_id'] . '&related_entities_id=' . $_GET['related_entities_id'] . '&fields_id=' . $_GET['fields_id']
        )
    );
    ...
    echo form_tag(
        'sorting_form',
        url_for('ext/funnelchart/view', 'id=' . $id . (strlen($app_path) ? '&path=' . $app_path : ''))
    );
} elseif (strstr($app_redirect_to, 'kanban')) {
    $id = str_replace('kanban', '', $app_redirect_to);
    echo form_tag(
        'sorting_form',
        url_for('ext/kanban/view', 'id=' . $id . (strlen($app_path) ? '&path=' . $app_path : ''))
    );
    ...
    <table width="100%">
        <tr>
            <td valign="top" width="45%">
                <fieldset>
                    <legend><?php
                        echo TEXT_FIELDS_FOR_SORTING ?></legend>
                            ...
                            if (count($sorting_fields) > 0) {
                                ...
                                if (count($sorting_fields) > 0) {
                                    $fields_query = db_query(
                                        "select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.id in (" . implode(
                                            ',',
                                            $sorting_fields
                                        ) . ") and f.type not in ('fieldtype_action') and  f.entities_id='" . db_input(
                                            $reports_info['entities_id']
                                        ) . "' and f.forms_tabs_id=t.id order by field(f.id," . implode(
                                            ',',
                                            $sorting_fields
                                        ) . ")"
                                    );
            ...
            <td style="padding-left: 25px;" valign="top">
                <fieldset>
                    <legend><?php
                        echo TEXT_FIELDS_EXCLUDED_FROM_SORTING ?></legend>
                    <div class="cfg_listing">
                        <ul id="fields_excluded_from_sorting" class="sortable">
                            ...
                            $fields_query = db_query(
                                "select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where " . (count(
                                    $sorting_fields
                                ) > 0 ? "f.id not in (" . implode(
                                        ',',
                                        $sorting_fields
                                    ) . ") and " : "") . " f.type not in (" . fields_types::get_type_list_excluded_in_sorting(
                                ) . ")  and  f.entities_id='" . db_input(
                                    $reports_info['entities_id']
                                ) . "' and f.forms_tabs_id=t.id {$where_sql} order by t.sort_order, t.name, f.sort_order, f.name"
                            );
                            while ($v = db_fetch_array($fields_query)) {
                                //check field access
                                if (isset($fields_access_schema[$v['id']])) {
...
<script>
    function prepare_condition_icons() {
        $('#fields_excluded_from_sorting .condition_icon').each(function () {
                    ...
                    if ($(this).attr('rel') == 'asc') {
                        $.ajax({
                            ...
                            url: '<?php echo url_for(
                                "reports/sorting",
                                "action=set_sorting_condition&reports_id=" . $_GET["reports_id"]
                            )?>',
                            data: {field_id: id, condition: 'desc'}
                        });
        ...
        $("ul.sortable").sortable({
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo url_for(
                        "reports/sorting",
                        "action=set_sorting&reports_id=" . $_GET["reports_id"]
                    )?>',
                    data: data
                });
            }
