<?php
echo ajax_modal_template_header(TEXT_NAV_LISTING_CONFIG) ?>
...
<div class="modal-body">
    ...
    <?php
    if (in_array($listing->get_listing_type(), ['table', 'tree_table'])): ?>
        <div><?php
                            ...
                            <ul id="fields_for_listing" class="sortable">
                                ...
                                if (count($fields_in_listing) > 0) {
                                    ...
                                    while ($v = db_fetch_array($fields_query)) {
                                        ...
                                        if ($v['type'] == 'fieldtype_parent_item_id' and ($app_entities_cache[$reports_info['entities_id']]['parent_id'] == 0 or strlen(
                                                    $app_path
                                                ))) {
                                            continue;
                                        }
    ...
    <?php
    echo TEXT_SHOW . ' ' . input_tag(
            'rows_per_page',
            ($reports_info['rows_per_page'] > 0 ? $reports_info['rows_per_page'] : CFG_APP_ROWS_PER_PAGE),
            ['class' => 'form-control form-control-inline input-xsmall']
        ) . ' <span style="text-transform: lowercase;">' . TEXT_ROWS_PER_PAGE . '.</span>' ?>
    ...
    $(function () {
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo url_for(
                        "reports/configure",
                        "action=set_listing_fields&reports_id=" . $_GET["reports_id"]
                    )?>',
                    data: data
                });
            }
        ...
        $('#rows_per_page').keyup(function () {
            $.ajax({
                ...
                url: '<?php echo url_for(
                    "reports/configure",
                    "action=set_rows_per_page&reports_id=" . $_GET["reports_id"]
                )?>',
                data: {rows_per_page: $(this).val()}
            });
        })
