<h3 class="form-title"><?php
    echo TEXT_HEADING_LDAP_LOGIN ?></h3>

<?php
echo(strlen(CFG_LOGIN_PAGE_CONTENT) > 0 ? '<p>' . nl2br(CFG_LOGIN_PAGE_CONTENT) . '</p>' : '') ?>

<?php
echo maintenance_mode::login_message() ?>

<?php
if (access_groups::get_ldap_default_group_id()) { ?>

    <?php
    echo form_tag('login_form', url_for('users/ldap_login', 'action=login'), ['class' => 'login-form']) ?>
    ...
    <div class="form-actions">
        <?php
        if (CFG_USE_LDAP_LOGIN_ONLY != 1): ?>
            <button type="button" id="back-btn" class="btn btn-default" onClick="location.href='<?php
            echo url_for('users/login') ?>'"><i class="fa fa-arrow-circle-left"></i></button>
        <?php
        endif ?>
