...
switch ($app_module_action) {
    case 'restore':
        ...
        $user_query = db_query(
            "select * from app_entity_1 where field_12='" . db_input($_POST['username']) . "' and field_9='" . db_input(
                $_POST['email']
            ) . "'"
        );
        if ($user = db_fetch_array($user_query)) {
            if ($user['field_5'] == 1) {
                ...
                $options = [
                    ...
                    'body' => TEXT_RESTORE_PASSWORD_EMAIL_BODY . '<p><b>' . TEXT_LOGIN_DETAILS . '</b></p><p>' . TEXT_USERNAME . ': ' . $user['field_12'] . '<br>' . TEXT_PASSWORD . ': ' . $password . '</p><p><a href="' . url_for(
                            'users/login',
                            '',
                            true
