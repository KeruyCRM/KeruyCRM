...
$html = '
	<div class="table-scrollable">
		...
		<thead>  
		  <tr>	
				<th>' . input_checkbox_tag('select_all_items', '', ['class' => 'select_all_items']) . '</th>
...
while ($item = db_fetch_array($items_query)) {
    ...
    $html .= '
            ...
            'items_' . $item['id'],
            $item['id'],
            ['class' => 'items_checkbox', 'checked' => in_array($item['id'], $app_selected_notification_items)]
        ) . '</td>
				<td style="white-space: normal;"><a href="' . url_for(
            'items/info',
            'path=' . $path_info['full_path']
        ) . '">' . users_notifications::render_icon_by_type($item['type']) . ' ' . $item['name'] . '</a></td>
