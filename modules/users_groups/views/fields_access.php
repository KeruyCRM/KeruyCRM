...
echo form_tag(
    ...
    url_for(
        ...
        'action=set_access&id=' . $users_groups_info['id'] . '&entities_id=' . $entity_info['id']
    )
) ?>
      ...
      <table class="table table-striped table-bordered table-hover">
        <tr>
          ...
          <th>' . TEXT_ACCESS . ': ' . select_tag(
                'access_' . $users_groups_info['id'],
                array_merge(['' => ''], $access_choices_default),
                '',
                [
                    'class' => 'form-control input-medium ',
                    'onChange' => 'set_access_to_all_fields(this.value,' . $users_groups_info['id'] . ')'
                ]
            ) . '</th>
        </tr>
      ';
        ...
        <tr>
          <td>' . $field['name'] . '</td>
          <td>' . select_tag(
                    'access[' . $users_groups_info['id'] . '][' . $id . ']',
                    $access_choices,
                    $value,
                    ['class' => 'form-control input-medium access_group_' . $users_groups_info['id']]
                ) . '</td>
        </tr>
      ';
        }
