...
$breadcrumb[] = '<li>' . link_to(
        TEXT_MENU_USERS_ACCESS_GROUPS,
        url_for('users_groups/users_groups')
...
<h3 class="page-title"><?php
    echo TEXT_PIVOT_ACCESS_TABLE . ' <i class="fa fa-angle-right"></i> ' . $users_groups_info['name'] ?></h3>

<?php
echo button_tag(TEXT_COPY_ACCESS, url_for('users_groups/copy_access', 'id=' . $users_groups_info['id'])) ?>

<?php
echo form_tag(
    'pivot_access_form',
    url_for('users_groups/pivot_access_table', 'action=set_access&id=' . $users_groups_info['id'])
) ?>
<div class="table-scrollable" style="overflow-x:visible;overflow-y:visible; ">
    <table class="table table-striped table-bordered table-hover">
        ...
        <tr>
            <th><?php
                echo input_checkbox_tag('select_all_items', '', ['class' => 'select_all_items']) ?></th>
            <th width="100%"><?php
                echo TEXT_ENTITY ?></th>
            <th><?php
        ...
        foreach (entities::get_tree() as $v):
            ...
            $comments_schema = '';
            $comments_acess_info_query = db_query(
            ...
            <tr>
                <td><?php
                    echo input_checkbox_tag('items[]', $v['id'], ['class' => 'items_checkbox']) ?></td>
                ...
                <td><?php
                    ...
                    echo '<div style="padding-top: 5px; text-align: right;">' . button_tag(
                            ...
                            url_for(
                                'users_groups/fields_access',
                                'id=' . $users_groups_info['id'] . '&entities_id=' . $v['id']
                            ),
                            true,
...
<?php
echo '<a class="btn btn-default" href="' . url_for('users_groups/users_groups') . '">' . TEXT_BUTTON_BACK . '</a>' ?>
...
<script>
    ...
    $(function () {
        ...
        $('.access-schema-settings').change(function () {
            ...
            $.ajax({
                type: "POST",
                url: form.attr('action') + '&entities_id=' + $(this).attr('data-entity-id'),
