        ...
        if (isset($_POST['copy_to_group_id'])) {
            ...
            foreach (explode(',', $_POST['selected_items']) as $entities_id) {
                ...
                $acess_info_query = db_query(
                    "select access_schema from app_entities_access where entities_id='" . $entities_id . "' and access_groups_id='" . $users_groups_info['id'] . "'"
                );
                if ($acess_info = db_fetch_array($acess_info_query)) {
                    $access_schema = $acess_info['access_schema'];
                }

                $sql_data = ['access_schema' => $access_schema];

                //update access
                $acess_info_copy_query = db_query(
                    "select access_schema from app_entities_access where entities_id='" . $entities_id . "' and access_groups_id='" . $copy_to_group_id . "'"
                );
                if ($acess_info_copy = db_fetch_array($acess_info_copy_query)) {
                    db_perform(
                        ...
                        $sql_data,
                        'update',
                        "entities_id='" . $entities_id . "' and access_groups_id='" . $copy_to_group_id . "'"
                    );
                } else {
                    //insert new access
                ...
                db_query(
                    "delete from app_fields_access where entities_id='" . db_input(
                        $entities_id
                    ) . "' and access_groups_id='" . db_input($copy_to_group_id) . "'"
                );

                //insert new fields access
                $sql_data = [];
                $fields_access_query = db_query(
                    "select * from app_fields_access where entities_id='" . db_input(
                        $entities_id
                    ) . "' and access_groups_id='" . db_input($users_groups_info['id']) . "'"
                );
                ...
                $sql_data = ['access_schema' => str_replace('_', ',', $access_schema)];

                $acess_info_query = db_query(
                    "select access_schema from app_comments_access where entities_id='" . db_input(
                        $entities_id
                    ) . "' and access_groups_id='" . $copy_to_group_id . "'"
                );
                if ($acess_info = db_fetch_array($acess_info_query)) {
                    db_perform(
                        'app_comments_access',
                        $sql_data,
                        'update',
                        "entities_id='" . db_input($entities_id) . "' and access_groups_id='" . $copy_to_group_id . "'"
                    );
                } else {
                    $sql_data['entities_id'] = $entities_id;
                    $sql_data['access_groups_id'] = $copy_to_group_id;
