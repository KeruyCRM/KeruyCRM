...
<div class="table-scrollable ">
  <div class="table-scrollable table-wrapper slimScroll" id="slimScroll">
    <table class="table table-striped table-bordered table-hover ' . ($listing->is_resizable(
    ) ? 'table-resizable' : '') . '" data-count-fixed-columns="' . reports::get_count_fixed_columns(
        $_POST['reports_id'],
        $has_with_selected
    ) . '" data-fixed-head="1" data-resizable="' . $listing->is_resizable() . '" ' . $listing->resizable_table_widht() . '>
      <thead>
        <tr class="listing-table-tr">
          ' . ($has_with_selected ? '<th class="multiple-select-action-th">' . $listing->select_all_choices(
            $listing_split->number_of_pages
        ) . '</th>' : '');
...
while ($v = db_fetch_array($fields_query)) {
    ...
    if ($v['type'] == 'fieldtype_dropdown_multilevel') {
        ...
        $html .= '
	      <th ' . $listing_order_action . ' ' . $listing_order_css_class . ' data-field-id="' . $v['id'] . '" ' . $listing->get_listing_col_width(
                $v['id']
            ) . '>
...
while ($item = db_fetch_array($items_query)) {
    $html .= '
      <tr class="listing-table-tr ' . (($users_notifications->has($item['id']) and $entity_cfg->get(
    ...
    foreach ($listing_fields as $field) {
        ...
        if ($field['is_heading'] == 1) {
            ...
            $html .= '
                ...
                ) . '  field-' . $field['id'] . '-td item_heading_td' . (($entity_cfg->get(
