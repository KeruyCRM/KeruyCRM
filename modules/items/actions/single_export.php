...
switch ($app_module_action) {
    case 'export':
        ...
        if (isset($_POST['fields'])) {
            ...
            $item_query = db_query(
                "select e.* " . $listing_sql_query_select . " from app_entity_" . $current_entity_id . " e where id='" . $current_item_id . "'"
            );
                ...
                while ($field = db_fetch_array($fields_query)) {
                    ...
                    if ($field['type'] == 'fieldtype_dropdown_multilevel') {
                        $export_data = array_merge(
                            $export_data,
                            fieldtype_dropdown_multilevel::output_info_box($output_options, true)
                        );
                    } else {
                        if (in_array(
                            $field['type'],
                        ...
                        $export_data[] = [fields_types::get_option($field['type'], 'name', $field['name']), $output];
                    }
                }
            ...
            if (users::has_comments_access(
                    'view'
                ) and $entity_cfg['use_comments'] == 1 and isset($_POST['export_comments'])) {
                $html_comments = '';

                $listing_sql = "select * from app_comments where entities_id='" . db_input(
                        $current_entity_id
                    ) . "' and items_id='" . db_input($current_item_id) . "'  order by date_added desc";
                ...
                while ($item = db_fetch_array($items_query)) {
                    ...
                    while ($field = db_fetch_array($comments_fields_query)) {
                        ...
                        $html_fields .= '                      
                <tr><th>&bull;&nbsp;' . $field['name'] . ':&nbsp;</th><td>' . trim(
            ...
            ';
                    ...
                    $attachments = fields_types::output(
                        [
                            ...
                            'path' => $current_path,
                            'field' => ['entities_id' => $current_entity_id],
            ...
            <tr>                  
              ...
              <td class="nowrap" valign="top">' . date(
                            CFG_APP_DATETIME_FORMAT,
                            $item['date_added']
                        ) . '<br>' . $app_users_cache[$item['created_by']]['name'] . '<br>' . '</td>
            </tr>
            <tr>
              <td colspan="2"><hr></td>
            </tr>
