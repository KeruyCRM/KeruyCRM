...
if (strlen($_POST['search_keywords']) > 0) {
    echo '<div class="alert alert-info">' . sprintf(
        ...
        ) . ' <span onClick="reset_search()" class="reset_search">' . TEXT_RESET_SEARCH . '</span></div>';

    require(component_path('items/add_search_comments_query'));
}


$user_has_comments_access = (users::has_comments_access(
        'update',
        $access_rules->get_comments_access_schema()
    ) or users::has_comments_access(
        'delete',
        $access_rules->get_comments_access_schema()
    ) or users::has_comments_access('create', $access_rules->get_comments_access_schema()));
...
?>
    ...
    <tr>
        <?php
        if ($user_has_comments_access) echo '<th>' . '</th>' ?>
        <?php
        if ($entity_cfg->get('display_comments_id') == 1) echo '<th>' . TEXT_ID . '</th>' ?>
        <th width="100%"><?php
            echo TEXT_COMMENTS ?></th>
        <th><?php
...
$listing_sql = "select * from app_comments where entities_id='" . db_input(
        $current_entity_id
    ) . "' and items_id='" . db_input($current_item_id) . "' " . $listing_sql_query . " order by id desc";
...
while ($item = db_fetch_array($items_query)) {
    ...
    if ($user_has_comments_access) {
        $html_action_column = '
      <td class="nowrap">
      ' . ((users::has_comments_access(
                    'delete',
                    $access_rules->get_comments_access_schema()
                ) and ($item['created_by'] == $app_user['id'] or $app_user['group_id'] == 0 or users::has_comments_access(
                        'full',
                        $access_rules->get_comments_access_schema()
                    ))) ? button_icon_delete(
                    url_for('items/comments_delete', 'id=' . $item['id'] . '&path=' . $_POST['path'])
                ) . '<br>' : '') . '
      ' . ((users::has_comments_access(
                    'update',
                    $access_rules->get_comments_access_schema()
                    ...
                    url_for('items/comments_form', 'id=' . $item['id'] . '&path=' . $_POST['path'])
                ) . '<br>' : '') . '
			' . (users::has_comments_access('create', $access_rules->get_comments_access_schema()) ? button_icon(
                TEXT_REPLY,
                'fa fa-reply',
                url_for('items/comments_form', 'reply_to=' . $item['id'] . '&path=' . $_POST['path'])
            ) : '') . '
      </td>
    ';
    }
    ...
    while ($field = db_fetch_array($comments_fields_query)) {
        ...
        $html_fields .= '                      
        <tr><th>&bull;&nbsp;' . fields_types::get_option(
                $field['type'],
                'name',
                $field['name']
            ) . ':&nbsp;</th><td>' . fields_types::output($output_options) . '</td></tr>           
    ';
    }
    ...
    $output_options = [
        ...
        'field' => [
            ...
            'configuration' => json_encode(['use_image_preview' => $entity_cfg->get('image_preview_in_comments', 0)])
        ],
    ...
    <tr>
      ...
      <td style="white-space: normal;">
        			<div class="ckeditor-images-content-prepare"><div class="fieldtype_textarea_wysiwyg">' . auto_link_text(
            $item['description']
        ) . '</div></div>' .
        $attachments .
        $html_fields .
