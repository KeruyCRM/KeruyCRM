...
<div class="modal-body">
    ...
    <div class="tab-content">
        <div class="tab-pane fade active in" id="select_fields_tab">
            ...
            <?php
            echo form_tag('export-form', url_for('items/single_export', 'action=export&path=' . $_GET['path'])) ?>
            ...
            <p>
                ...
                while ($tabs = db_fetch_array($tabs_query)) {
                    ...
                    $fields_query = db_query(
                        "select f.* from app_fields f where  f.type not in ('fieldtype_action') and f.entities_id='" . db_input(
                            $current_entity_id
                        ) . "' and forms_tabs_id='" . db_input($tabs['id']) . "' order by f.sort_order, f.name"
                    );
            ...
            <?php
            if (users::has_comments_access('view') and $entity_cfg['use_comments'] == 1): ?>
                <p><label><?php
                        echo input_checkbox_tag(
                                'export_comments',
                                'comments',
                                ['class' => 'export_fields export_fields_comments']
                            ) . ' ' . TEXT_EXPORT_COMMENTS ?></label></p>
            <?php
            endif ?>

            <p><?php
                echo TEXT_FILENAME . '<br>' . input_tag(
                        'filename',
                        items::get_heading_field($current_entity_id, $current_item_id),
