...
echo form_tag(
    ...
    url_for('items/related_item', 'action=remove_related_items&path=' . $_GET['path'])
) ?>
    ...
    while ($items = db_fetch_array($items_query)) {
        ...
        if (strlen($related_records->cfg->get('heading_template'))) {
            ...
            $item_name = $text_pattern->output_singe_text(
                $related_records->cfg->get('heading_template'),
                $related_records->cfg->get('entity_id'),
                $items
            );
        } else {
        ...
        echo '<div>' . input_checkbox_tag('items[]', $related_id, ['class' => 'remove_related_item']
            ) . ' <a href="' . url_for(
                'items/info',
