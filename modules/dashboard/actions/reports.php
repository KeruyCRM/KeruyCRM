...
if ($app_reports_groups_id > 0) {
    $reports_groups_info_query = db_query(
        "select * from app_reports_groups where ((created_by = '" . $app_user['id'] . "' and is_common=0) or (" . ($app_user['group_id'] > 0 ? "find_in_set(" . $app_user['group_id'] . ",users_groups) and " : "") . " is_common=1)) and id='" . $app_reports_groups_id . "'"
    );
...
switch ($app_module_action) {
    ...
    case 'sort_reports':
        if (isset($_POST['reports_on_dashboard'])) {
            $sql_data = ['reports_list' => str_replace('report_', '', $_POST['reports_on_dashboard'])];
    ...
    case 'sort_reports_counter':
        if (isset($_POST['reports_counter_on_dashboard'])) {
            $sql_data = ['counters_list' => str_replace('report_', '', $_POST['reports_counter_on_dashboard'])];
            db_perform('app_reports_groups', $sql_data, 'update', "id='" . $app_reports_groups_id . "'");
        }
    ...
    case 'add_section':
        $sql_data = [
            'reports_groups_id' => $app_reports_groups_id,
            'created_by' => $app_user['id'],
            'count_columns' => _POST('columns')
        ];
    ...
    case 'edit_section':
        if (isset($_POST['section_id'])) {
            ...
            $check_query = db_query(
                "select id from app_reports_sections where ((report_left='{$value}' and length(report_left)>0) or (report_right='{$value}' and length(report_right)>0)) and reports_groups_id={$app_reports_groups_id} and created_by='{$app_user['id']}'"
            );
            if (!$check = db_fetch_array($check_query)) {
                $sql_data = [$_POST['type'] => $value];
        ...
        if (isset($_POST['section_panel'])) {
            ...
            foreach (explode(',', $_POST['section_panel']) as $v) {
                ...
                db_perform(
                    ...
                    $sql_data,
                    'update',
                    "id='" . db_input(str_replace('section_panel_', '', $v)) . "'"
                );
                $sort_order++;
            }
        }
