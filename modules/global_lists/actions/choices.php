...
switch ($app_module_action) {
    case 'save':
        ...
        if (isset($_POST['is_default'])) {
            db_query(
                "update app_global_lists_choices set is_default = 0 where lists_id = '" . db_input(
                    $_GET['lists_id']
                ) . "'"
            );
        }
    ...
    case 'multiple_delete':
        ...
        if (strlen($_POST['selected_fields'])) {
            db_query(
                "delete from app_global_lists_choices where lists_id='" . _get::int(
                    'lists_id'
                ) . "' and id in (" . $_POST['selected_fields'] . ")"
            );
            ...
            $choices_query = db_query(
                "select * from app_global_lists_choices c where (select count(*) from  app_global_lists_choices c2 where c2.id=c.parent_id)=0"
            );
            ...
            if (count($reset_parents_id)) {
                db_query(
                    "update app_global_lists_choices set parent_id=0 where id in (" . implode(
                        ',',
                        $reset_parents_id
                    ) . ")"
                );
    ...
    case 'sort_reset':
        ...
        db_query(
            "update app_global_lists_choices set sort_order = 0 where lists_id = '" . db_input(_GET('lists_id')) . "'"
        );
                    ...
                    for ($col; $col <= $import_columns; $col++) {
                        ...
                        if (strlen($value) and $check_parent_name[$col] != $value) {
                            ...
                            $check_query = db_query(
                                "select id from app_global_lists_choices where lists_id='" . _GET(
                                    'lists_id'
                                ) . "' and parent_id='" . $parent_id[$col] . "' and name='" . db_input($value) . "'"
                            );
