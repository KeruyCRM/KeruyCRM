...
<h3 class="page-title"><?php
    echo TEXT_NAV_COMMENTS_FORM_CONFIG ?></h3>

<p><?php
    echo TEXT_COMMENTS_FORM_CFG_INFO ?></p>
...
<table style="width: 100%; max-width: 960px;">
    <tr>
        <td valign="top" width="50%">
            ...
            <fieldset>
                <legend><?php
                    echo TEXT_FIELDS_IN_COMMENTS_FORM ?></legend>
                <div class="cfg_listing">
                    <ul id="fields_in_comments" class="sortable">
                        <?php
                        $fields_query = db_query(
                            "select f.* from app_fields f where  comments_status = 1 and  f.entities_id='" . db_input(
                                $_GET['entities_id']
                            ) . "' and f.comments_forms_tabs_id=0 order by f.comments_sort_order"
                        );
                        while ($v = db_fetch_array($fields_query)) {
                            echo '<li id="fields_' . $v['id'] . '"><div>' . fields_types::get_option(
                                    $v['type'],
                                    'name',
                                    $v['name']
                                ) . '</div></li>';
            ...
            echo button_tag(
                ...
                url_for('entities/comments_forms_tabs_form', 'entities_id=' . $_GET['entities_id'])
            ) ?>
                                            ...
                                            <tr>
                                                ...
                                                <td class="align-right">
                                                    ...
                                                    echo button_icon_edit(
                                                            url_for(
                                                                'entities/comments_forms_tabs_form',
                                                                'id=' . $tabs['id'] . '&entities_id=' . $_GET['entities_id']
                                                            )
                                                        ) . ' ' . button_icon_delete(
                                                            url_for(
                                                                'entities/comments_forms_tabs_delete',
                                                                'id=' . $tabs['id'] . '&entities_id=' . $_GET['entities_id']
                                                            )
                                                        ); ?>
                                                </td>
                    ...
                    <ul id="available_fields" class="sortable">
                        ...
                        $fields_query = db_query(
                            "select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in ('" . implode(
                                "','",
                                comments::get_available_filedtypes_in_comments()
                            ) . "') and  comments_status = 0 and  f.entities_id='" . db_input(
                                $_GET['entities_id']
    ...
    $(function () {
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo url_for(
                        "entities/comments_form",
                        "action=set_fields&entities_id=" . $_GET["entities_id"]
                    )?>',
                    data: data
                });
            }
        ...
        $("ol.sortable_tabs").sortable({
            ...
            update: function (event, ui) {
                ...
                $.ajax({
                    type: "POST",
                    url: '<?php echo url_for("entities/comments_form", "action=sort_tabs")?>',
                    data: data
                });
            }
        });
