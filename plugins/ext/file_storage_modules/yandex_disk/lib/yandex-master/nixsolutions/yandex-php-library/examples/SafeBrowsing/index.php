...
<div class="container">
    <div class="jumbotron">
        <h2><span class="glyphicon glyphicon glyphicon-certificate"></span> Пример работы с Safe Browsing API Яндекса
        </h2>
    </div>
    <ol class="breadcrumb">
    ...
    try {
        ...
        if (isset($_GET['url']) && $_GET['url']) {
            ...
            if ($safeBrowsing->searchUrl($url)) {
                ?>
                <div class="alert alert-danger">Найден полный хеш для "<?= htmlentities($url) ?>" в списке опасных
                    сайтов
                </div>
                <?php
            } else {
