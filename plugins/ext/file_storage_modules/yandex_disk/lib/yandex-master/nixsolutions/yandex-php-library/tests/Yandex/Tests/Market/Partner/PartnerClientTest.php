...
class PartnerClientTest extends TestCase
{
    ...
    public function testGetStats()
    {
        ...
        $statsResp = $marketPartnerMock->getStats(
            'main',
            ['fromDate' => '06-02-2017', 'fields' => 'mobile,shows', 'toDate' => '08-02-2017']
        )->getAll();
    ...
    public function testGetOutlets()
    {
        ...
        $this->assertEquals(
            $outletsObj[0]->deliveryRules[0]->dateSwitchHour,
            $outletFirstDeliveryRule->getDateSwitchHour()
        );
        $this->assertEquals(
            $outletsObj[0]->deliveryRules[0]->minDeliveryDays,
            $outletFirstDeliveryRule->getMinDeliveryDays()
        );
        $this->assertEquals(
            $outletsObj[0]->deliveryRules[0]->maxDeliveryDays,
            $outletFirstDeliveryRule->getMaxDeliveryDays()
        );
        ...
        $this->assertEquals(
            $outletsObj[0]->deliveryRules[0]->shipperHumanReadableId,
            $outletFirstDeliveryRule->getShipperHumanReadableId()
        );
        ...
        $this->assertEquals(
            $outletsObj[0]->deliveryRules[0]->unspecifiedDeliveryInterval,
            $outletFirstDeliveryRule->getUnspecifiedDeliveryInterval()
        );
        $this->assertEquals(
            $outletsObj[0]->deliveryRules[0]->workInHoliday,
            $outletFirstDeliveryRule->getWorkInHoliday()
        );
