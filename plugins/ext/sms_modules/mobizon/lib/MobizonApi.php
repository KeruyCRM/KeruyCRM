...
class MobizonApi
{
     ...
     */
    private static $allowedConstructorParams = [
        'apiServer',
        'apiKey',
        'forceHTTP',
        'skipVerifySSL',
     ...
     */
    private static $allowedFormats = ['xml', 'json'];
    ...
    public function __construct($params = [])
    {
        ...
        if (!$this->forceHTTP && !in_array('openssl', get_loaded_extensions())) {
            throw new Mobizon_OpenSSL_Required(
                'The OpenSSL extension is required but not currently enabled. Install OpenSSL or set forceHTTP=true in params to switch to insecure connection.'
            );
        }
        ...
        switch ($key) {
            case 'format':
                ...
                if (!in_array($value, static::$allowedFormats)) {
                    throw new Mobizon_Error(
                        'Format should be one of the following: ' . implode(
                            ', ',
    ...
    public function call(
        ...
        $url = ($this->forceHTTP ? 'http' : 'https') . '://' . $this->apiServer . '/service/' . strtolower(
                $provider
            ) . '/' . strtolower($method) . '?';
        ...
        return $returnData ? $this->getData() : (in_array($this->getCode(), [0, 100]));
    }
