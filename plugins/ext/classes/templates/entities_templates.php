...
class entities_templates
{
        ...
        if (count($templates_list = self::get_users_templates($entities_id)) > 0) {
            foreach ($templates_list as $template) {
                ...
                $templates_fields_query = db_query(
                    "select tf.id, tf.fields_id, tf.value, f.name, f.type from app_ext_entities_templates_fields tf, app_fields f left join app_forms_tabs t on f.forms_tabs_id=t.id  where f.id=tf.fields_id and tf.templates_id='" . db_input(
                        $template['id']
    ...
    static function get_users_templates($entities_id)
    {
        ...
        $templates_query = db_query(
            "select ep.* from app_ext_entities_templates ep, app_entities e where ep.is_active=1 and e.id=ep.entities_id and ep.entities_id='" . db_input(
                $entities_id
        ...
        while ($templates = db_fetch_array($templates_query)) {
            $templates_list[] = ['id' => $templates['id'], 'name' => $templates['name']];
        }
