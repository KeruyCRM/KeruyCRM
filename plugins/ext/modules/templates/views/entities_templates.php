...
$templates_query = db_query(
    "select ep.*, e.name as entities_name from app_ext_entities_templates ep, app_entities e where e.id=ep.entities_id " . $where_sql . " order by e.id, ep.sort_order, ep.name"
);
?>
...
<div class="row">
    <div class="col-md-9">
        <?php
        echo button_tag(TEXT_BUTTON_CREATE, url_for('ext/templates/entities_templates_form'), true) ?>
        <?php
        if (db_num_rows($templates_query) > 1 and $entities_templates_filter > 0) echo button_tag(
            TEXT_SORT_ORDER,
            url_for('ext/templates/entities_templates_sort'),
            true,
    ...
    <div class="col-md-3">
        ...
        echo form_tag(
            'templates_filter',
            url_for('ext/templates/entities_templates', 'action=set_entities_templates_filter')
        ) ?>
        <?php
        echo select_tag(
            'entities_templates_filter',
            ...
            <tr>
                <td style="white-space: nowrap;"><?php
                    echo button_icon_delete(
                            url_for('ext/templates/entities_templates_delete_confirm', 'id=' . $templates['id'])
                        ) . ' ' . button_icon_edit(
                            url_for('ext/templates/entities_templates_form', 'id=' . $templates['id'])
                        ) ?></td>
                <td><?php
