...
$breadcrumb[] = '<li>' . link_to(
        $reports['name'],
        url_for('ext/pivot_tables/view', 'id=' . $_GET['id'])
    ) . '<i class="fa fa-angle-right"></i></li>';
...
$pivotreports_fields_query = db_query(
    "select * from app_ext_pivot_tables_fields where reports_id='" . db_input($_GET['id']) . "'"
);
    ...
    while ($fields = db_fetch_array($fields_query)) {
        $html .= '
                ...
                ['class' => 'form-control input-medium']
					...
					<td>' . (in_array(
                $fields['type'],
                ['fieldtype_date_added', 'fieldtype_input_date', 'fieldtype_input_datetime', 'fieldtype_dynamic_date']
            ) ?
                input_tag(
