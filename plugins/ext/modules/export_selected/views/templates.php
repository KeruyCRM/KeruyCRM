<h3 class="page-title"><?php
    echo TEXT_EXT_EXPORT_SELECTED ?></h3>

<p><?php
    echo TEXT_EXT_EXPORT_SELECTED_INFO ?></p>
...
$templates_query = db_query(
    "select ep.*, e.name as entities_name from app_ext_export_selected ep, app_entities e where e.id=ep.entities_id " . $where_sql . " order by e.id, ep.sort_order, ep.name"
);
?>
...
<div class="row">
    <div class="col-md-9">
        <?php
        echo button_tag(TEXT_BUTTON_CREATE, url_for('ext/export_selected/form'), true) ?>
        <?php
        if (db_num_rows($templates_query) > 1 and $export_templates_filter > 0) echo button_tag(
            TEXT_SORT_ORDER,
            url_for('ext/export_selected/sort'),
            true,
    ...
    <div class="col-md-3">
        ...
        echo form_tag(
            'templates_filter',
            url_for('ext/export_selected/templates', 'action=set_export_templates_filter')
        ) ?>
        <?php
        echo select_tag(
            ...
            <tr>
                <td style="white-space: nowrap;"><?php
                    echo button_icon_delete(
                            url_for('ext/export_selected/delete_confirm', 'id=' . $templates['id'])
                        ) . ' ' .
                        button_icon_edit(url_for('ext/export_selected/form', 'id=' . $templates['id'])) . ' ' .
                        button_icon(
                            TEXT_COPY,
                            'fa fa-files-o',
                            url_for('ext/export_selected/templates', 'action=copy&templates_id=' . $templates['id']),
                            false,
                ...
                <td><?php
                    ...
                    if (in_array($templates['type'], ['docx'])) {
                        $html = link_to(
                            $templates['name'],
                            url_for('ext/export_selected/table_blocks', 'templates_id=' . $templates['id'])
                        );
