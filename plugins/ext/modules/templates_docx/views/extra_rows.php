...
echo button_tag(
    TEXT_BUTTON_ADD,
    url_for(
        'ext/templates_docx/extra_rows_form',
        'templates_id=' . $template_info['id'] . '&parent_block_id=' . $parent_block['id'] . '&row_id=' . $row_info['id']
    )
) ?>
...
<div class="table-scrollable">
    <table class="table table-striped table-bordered table-hover">
        ...
        $blocks_query = db_query(
            "select b.* from app_ext_items_export_templates_blocks b where b.templates_id = " . $template_info['id'] . " and b.parent_id = " . $row_info['id'] . " order by b.sort_order, b.id",
            false
        );
            ...
            <tr>
                <td style="white-space: nowrap;"><?php
                    echo button_icon_delete(
                            url_for(
                                'ext/templates_docx/extra_rows_delete',
                                'id=' . $blocks['id'] . '&templates_id=' . $template_info['id'] . '&parent_block_id=' . $parent_block['id'] . '&row_id=' . $row_info['id']
                            )
                        ) . ' ' . button_icon_edit(
                            url_for(
                                'ext/templates_docx/extra_rows_form',
