    ...
    case 'add_row':
        ...
        if ($_GET['block_type'] == 'thead') {
            $sort_order_query = db_query(
                "select (min(sort_order)-1) as sort_order from app_ext_items_export_templates_blocks where block_type='" . $_GET['block_type'] . "' and templates_id=" . $template_info['id'] . " and parent_id=" . $parent_block['id']
            );
        } else {
            $sort_order_query = db_query(
                "select (max(sort_order)+1) as sort_order from app_ext_items_export_templates_blocks where block_type='" . $_GET['block_type'] . "' and templates_id=" . $template_info['id'] . " and parent_id=" . $parent_block['id']
            );
        }
