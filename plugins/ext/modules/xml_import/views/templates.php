<h3 class="page-title"><?php
    echo TEXT_EXT_XML_IMPORT ?></h3>

<p><?php
    echo TEXT_EXT_XML_IMPORT_INFO ?></p>
...
$templates_query = db_query(
    "select ep.*, e.name as entities_name from app_ext_xml_import_templates ep, app_entities e where e.id=ep.entities_id " . $where_sql . " order by e.id, ep.sort_order, ep.name"
);
?>
...
<div class="row">
    <div class="col-md-9">
        <?php
        echo button_tag(TEXT_BUTTON_CREATE, url_for('ext/xml_import/form'), true) ?>
    </div>
    <div class="col-md-3">
        <?php
        echo form_tag('templates_filter', url_for('ext/xml_import/templates', 'action=set_xml_templates_filter')) ?>
        <?php
        echo select_tag(
            ...
            <tr>
                <td style="white-space: nowrap;"><?php
                    echo button_icon_delete(
                            url_for('ext/xml_import/delete_confirm', 'id=' . $templates['id'])
                        ) . ' ' . button_icon_edit(url_for('ext/xml_import/form', 'id=' . $templates['id'])) ?></td>
                <td><?php
                    echo $templates['id'] ?></td>
                <td><?php
                ...
                <td style="white-space: normal">
                    ...
                    if (strlen($templates['filepath'])) {
                        echo tooltip_text(
                            '<small>' . TEXT_EXT_IMPORT_BY_URL . ': <a href="' . url_for(
                                'ext/xml_import/import_preview',
                                'id=' . $templates['id']
                            ) . '">' . TEXT_EXT_XML_IMPORT_PREVIEW . '</a></small>'
                        );
                    }
                    ?>
                </td>
